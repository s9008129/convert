...
# ---------------------------------------------------------------------------
# T-20 ~ T-22：E1 實測修補（真實 gemma-4-31B-it-MLX-4bit 筆記格式）
# 證據：`.agent/tasks/T20260922-2037-02-local-model-quality-parity/e2e/attempt-E1-gemma31b-p4/`
# （E1 run log `cov_expected_topic=0`／`cov_missing_decision=11`；離線重播見 run_notes）
# ---------------------------------------------------------------------------


def test_T20_議題抽取_整行粗體標題形式_E1實測回歸():
    """E1 實測：真實筆記把議題寫成**整行粗體標題**，不是 `- **議題**：X`。

    舊行為：`- **組織規程與編制變動（11月1日生效）**` 不符 marker 樣式 → 議題期望
    集合恆為 0 → 該類別永遠 no-op（E1 log `cov_expected_topic=0`），
    而議題正是 67 條清單裡最大的缺口類別（topic 覆蓋率 0.36→0.4）。
    """
    notes = (
        "## 2. 議題與決議\n"
        "- **組織規程與編制變動（11月1日生效）**\n"
        "  - *討論重點*：土地稅科分拆為「地價稅科」與「土地增值稅科」。\n"
        "  - *決議*：[00:00:00] 發言者 1（主席裁示）：資管股需預先準備設備。\n"
        "- **瑞理發放活動（10月14日）**\n"
        "  - *決議*：[00:06:14] 發言者 1（主席裁示）：\n"
        "    - 確認發放日期為 10/14。\n"
    )

    topics = SummarizationService._extract_notes_topic_items(notes)
    decisions = SummarizationService._extract_notes_decision_items(notes)
...
def test_T21_決議抽取_引用標頭剝除_E1實測假陽性回歸():
    """E1 實測：決議條目帶逐字稿引用標頭，LCS 被標頭吃掉一半 → 已涵蓋卻判遺漏。

    離線重播（E1 真實筆記 × E1 真實紀錄）：舊版 `cov_missing_decision=11`
    （11/11 全判缺，人工核對其中 7 項其實已在紀錄中）→ 補強註定不收斂、
    白燒 2 輪（≈970 s）。剝除標頭後同一組輸入 `cov_missing_decision=3`。
    """
    notes = (
        "## 2. 議題與決議\n"
        "- **土地稅卡重新列印**\n"
        "  - *決議*：[00:08:15] 發言者 1（主席）裁示："
        "土地稅科整理出需重印之名單後，由本科配合重新列印。\n"
    )
...
def test_T22_引用標頭剝除_純函式邊界():
    """標頭剝除＝純函式、冪等、無模型名分支；沒有標頭時輸出＝輸入。"""
    strip = SummarizationService._strip_notes_item_quote_prefix

    assert strip("[00:08:15] 發言者 1（主席）裁示：土地稅科整理名單。") == "土地稅科整理名單。"
    assert strip("[00:00:00-00:05:36] 發言者 12（科長）：確認日期。") == "確認日期。"
    assert strip("  [00:21:01] 發言者 1（主席裁示）：  ") == "", "空殼必須被剝成空字串"
    assert strip("討論重點：主席提醒廉政風險。") == "討論重點：主席提醒廉政風險。"
    assert strip("土地稅科整理名單。") == "土地稅科整理名單。"
    assert strip("") == ""
    assert strip(strip("[00:08:15] 發言者 1（主席）裁示：土地稅科整理名單。")) == "土地稅科整理名單。"
...
def test_T36_巢狀區塊內無冒號角色標籤不得被收成決議_同角色重述才是內容():
    """獨立稽核（P6-A 對抗性複核）構造出的假遺漏來源：

    巢狀區塊內出現**省略冒號**的角色標籤（`- *討論重點*`）時，若只認「有冒號」
    的標頭，該標籤與其子樹會被收成本標記的內容 → 產生假的「決議遺漏」補強問題
    （`決議遺漏 2 項：*討論重點*、土地稅科拆分…`），在 enforce 模式下會把
    討論重點污染成決議。同日誌：**同角色**的巢狀重述（`- 決議：通過`）本身
    是內容，不得因為它是標頭就被丟掉。
    """
    notes = (
        "## 2. 議題與決議\n"
    ...
    same_role = (
        "## 2. 議題與決議\n"
        "- **決議**：\n"
        "  - 決議：搬遷案照案通過，10月31日前完成點交。\n"
        "  - 決議事項：主辦人由抽籤決定，去年辦過者免辦。\n"
    )
    assert SummarizationService._extract_notes_decision_items(same_role) == [
        "搬遷案照案通過，10月31日前完成點交。",
        "主辦人由抽籤決定，去年辦過者免辦。",
    ], "同角色的巢狀重述是內容，不是別人家的標頭"


def test_T37_角色詞當內容開頭不得被切開_空殼議題仍跳過討論重點子樹():
    """標頭判定必須要求「角色詞後接冒號或空白」：`- 決議文康活動形式為…`／
    `- 討論重點清單需附於紀錄後。` 都是內容（不得被切成 `文康活動形式…`）。

    另一面：`- **議題**`（整行粗體＝標記詞、無冒號）是空殼議題，收巢狀時仍須
    跳過 `*討論重點*` 子樹（不得把討論收成議題）。
    """
    notes = (
        "## 2. 議題與決議\n"
        "- **決議**：\n"
    ...
    assert SummarizationService._extract_notes_topic_items(topic_shell) == [
        "辦公室搬遷與除黴工程進度確認。"
    ]
