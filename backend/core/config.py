...
class Settings(BaseSettings):
    ...
    # ========================================
    # P7 波（T20260923-1700-03）：地端補強輪「不回退」守衛
    # 根因背景：補強輪＝整份紀錄重生成。實測（gemma-4-31B-it-MLX-4bit、同一支音檔、同一模板）
    # 同一場三次對帳＝數字未涵蓋 3 項 → 0 項 → 又是 1 項，也就是「第 1 輪補回的事實、
    # 第 2 輪被寫掉」。交付的最後一版因此可能比中途某輪更差。
    # ========================================
    LOCAL_LLM_REFINEMENT_NO_REGRESSION: bool = Field(
        default=True,
        description="地端補強輪不回退守衛（P7-A）：補強每輪都是整份重生成，若某輪造成事實回退"
                    "——未涵蓋數變多，或未涵蓋數相同但把原本已寫到的事實換掉（等量互換）"
                    "——該輪輸出不採用，交付本次執行中核心覆蓋率最佳的那一版（同分取最後一版）。"
                    "比較只在 LOCAL_LLM_RECORD_COVERAGE_MODE != off 且期望集合非空時生效；"
                    "在 observe 模式下逐條對帳仍不產生補強問題（問題清單零變化），但交付版本"
