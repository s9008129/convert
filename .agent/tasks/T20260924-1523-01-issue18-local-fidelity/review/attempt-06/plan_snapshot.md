# Issue #18 Local Meeting Record Architecture V2 — Plan Revision 7
...
Add separate Qwen/Gemma controls only after inspecting backend/loaded model capabilities. This wiring and candidate comparison is required for acceptance/task closure, not for accepting an unrelated record. Treat these as starting candidates, not presumed truths: Qwen production baseline `thinking=OFF, temperature=0.7, top_p=0.8, top_k=20`; Qwen extraction A/B temperatures `0.3, 0.5, 0.7` with all other conditions fixed; Gemma independent baseline `thinking=OFF, temperature=1.0, top_p=0.95, top_k=64`. Verify accepted parameters from the active runtime and record explicit unsupported-setting diagnostics; an unsupported control is not a valid candidate sample and may not be silently counted as matching the requested profile. If model/runtime is unavailable, set the affected C9/C14 `CHECK_RESULT: BLOCKED` with `ENVIRONMENT` scope; do not count it as a failure or pass. If frozen evaluator/rubric/original unrounded baseline is unavailable, set affected C10/C14 `CHECK_RESULT: BLOCKED` with the exact quality blocker label; do not substitute rounded baselines or self-waive. Preserve implementation and CORE facts. When any such required item is blocked but other required verification produced valid evidence, aggregate `REQUIRED_VERIFICATION_STATUS: INCOMPLETE`; use `BLOCKED` only if the verification phase as a whole cannot obtain a valid conclusion. Before Stage05, report `INDEPENDENT_ACCEPTANCE_STATUS: PENDING` and `TASK_CLOSURE_STATUS: PENDING_REQUIRED_VERIFICATION` while required verification remains incomplete. If Stage05 cannot obtain a valid quality/E2E conclusion due the same authority/environment blocker, report `INDEPENDENT_ACCEPTANCE_STATUS: BLOCKED`, preserve Stage04's verification result, and use `TASK_CLOSURE_STATUS: ACCEPTANCE_BLOCKED`. No implementation status is downgraded or blocked solely by these acceptance prerequisites; continue unrelated work. For each candidate profile, run N≥5 when practical; N=3 is the minimum when measured time/hardware cost is excessive, with that limitation recorded. Compare median, worst case, range and hard-fail count. Select by priority: (1) major fidelity hard fails, (2) causal correctness, (3) attribution correctness, (4) faithfulness, (5) traceability, (6) completeness, (7) usability. Keep diagnostic runs separate from the fresh blind cohort. Determine context from active loaded instance; retrieve relevant evidence before increasing context. Quality policy cannot branch by family. Never use temperature 0 as production default.

### W8 — Verification, E2E, acceptance

Run focused synthetic suite with live-boundary regressions and v4.7.4 set; full `uv run pytest tests/ -q`, `bash scripts/check_docs.sh`, `git diff --check`. First reproduce/reject the exact Stage05 C1 predicate inversion through the live gate, then run selected-claim E2E through source→structured extraction→ledger→section render→delivery. Helper-only A–I tests do not prove C1. Then fresh Gemma and Qwen E2E. Complete C14 repeated candidate-profile sampling before final profile selection; report sample counts and median/worst/range/hard-fail count. These are CORE closure/independent-acceptance evidence, not per-record gates. For every missing evaluator/rubric/original unrounded baseline or unavailable model/hardware, record the affected C9/C10/C14 item as `CHECK_RESULT: BLOCKED` with `AUTHORITY` or `ENVIRONMENT` scope respectively; preserve implementation/CORE evidence, and never substitute rounded baselines or self-waive. If other required checks have valid evidence, aggregate `REQUIRED_VERIFICATION_STATUS: INCOMPLETE`; set `BLOCKED` only when the entire required-verification phase cannot obtain a valid conclusion. At Stage04 close, use `INDEPENDENT_ACCEPTANCE_STATUS: PENDING` and `TASK_CLOSURE_STATUS: PENDING_REQUIRED_VERIFICATION` when required verification is incomplete. At Stage05, if the scoped blocker prevents valid acceptance, preserve the Stage04 verification result, report `INDEPENDENT_ACCEPTANCE_STATUS: BLOCKED`, and route to `TASK_CLOSURE_STATUS: ACCEPTANCE_BLOCKED`. Separately run blind 3+3 with frozen rubric and original unrounded Gemini baseline; diagnostic/profile-selection runs are excluded. Show per-output/per-dimension minimum, median, worst, range and hard-fail count. Held-out different-template/meeting evidence only if an approved privacy-safe source exists.
...
| C9 Gemma/Qwen fresh E2E | CORE | OUTCOME | HARD_CLEAN | NO | Selected-claim failure or hidden fallback is FAIL; model/hardware unavailable after safe load/availability check is `CHECK_RESULT: BLOCKED` (`ENVIRONMENT`). Preserve implementation/CORE facts; required verification is `INCOMPLETE` if other required evidence exists, otherwise phase `BLOCKED`; Stage05 records independent acceptance `BLOCKED` and closure `ACCEPTANCE_BLOCKED` if no valid E2E result can be obtained | NO | NONE | NOT_RUN | NOT_ALLOWED |
| C10 blind 3+3 frozen rubric + zero major hard-fails | CORE | OUTCOME | HARD_CLEAN | YES | Per-model every output/dimension ≥ frozen Gemini median×0.80 and zero major failures; missing evaluator/rubric/original unrounded baseline is `CHECK_RESULT: BLOCKED` (`AUTHORITY`) with `QUALITY_ACCEPTANCE_BLOCKED_BY_MISSING_FROZEN_EVALUATOR`, never a guessed PASS/FAIL or waiver. Preserve implementation/CORE facts; aggregate required verification is `INCOMPLETE` when other required evidence exists, or phase `BLOCKED` only when none can yield a valid conclusion; Stage05 reports independent acceptance `BLOCKED` and closure `ACCEPTANCE_BLOCKED` if the missing authority prevents valid acceptance | NO | NONE | NOT_RUN | NOT_ALLOWED |
...
| C14 repeated candidate-profile sampling and selection | CORE | OUTCOME | HARD_CLEAN | YES | Live runtime exposes/validates candidate settings; per candidate N≥5 preferred/N=3 minimum with limitation; compare median/worst/range/hard-fail count and select by W7 priorities. Required for independent acceptance/task closure only, never an individual-record gate. Missing evaluator/rubric/original unrounded baseline is `CHECK_RESULT: BLOCKED` (`AUTHORITY`) with `QUALITY_ACCEPTANCE_BLOCKED_BY_MISSING_FROZEN_EVALUATOR`; unavailable model/hardware is `CHECK_RESULT: BLOCKED` (`ENVIRONMENT`). Preserve implementation/CORE facts; aggregate required verification is `INCOMPLETE` when other required evidence exists, or phase `BLOCKED` only when no required evidence can yield a valid conclusion; Stage05 records independent acceptance `BLOCKED` and closure `ACCEPTANCE_BLOCKED` if these prevent valid acceptance | NO | NONE | NOT_RUN | NOT_ALLOWED |
...
- Blocking to quality acceptance only: missing frozen evaluator/rubric or original unrounded baseline sets the affected C10/C14 `CHECK_RESULT: BLOCKED`, class `AUTHORITY`, exact label `QUALITY_ACCEPTANCE_BLOCKED_BY_MISSING_FROZEN_EVALUATOR`, and no waiver. This does not block implementation, regression, or available local E2E. If other required evidence exists, `REQUIRED_VERIFICATION_STATUS: INCOMPLETE`; if Stage05 cannot obtain valid acceptance, `INDEPENDENT_ACCEPTANCE_STATUS: BLOCKED` and `TASK_CLOSURE_STATUS: ACCEPTANCE_BLOCKED` while preserving the Stage04 snapshot.
- Environment-scoped acceptance blockers: requested model/hardware unavailable sets the affected C9/C14 `CHECK_RESULT: BLOCKED`, class `ENVIRONMENT`. Complete all unrelated work and preserve prior implementation/CORE evidence. Required-verification and Stage05 closure routing follows the same v4.2 rule above; never convert the model blocker to `IMPLEMENTATION_STATUS: BLOCKED`.
...
Profile-selection acceptance additionally requires repeated candidate-profile runs at the counts and statistics in W7/C14; these required closure/independent-acceptance runs never veto an unrelated record and never count as blind 3+3 samples. A blocked C9/C10/C14 item keeps its `CHECK_RESULT: BLOCKED`; with other valid required evidence the aggregate is `REQUIRED_VERIFICATION_STATUS: INCOMPLETE`, while no valid conclusion across the required-verification phase yields aggregate `BLOCKED`. Before Stage05, unresolved required verification yields `PENDING_REQUIRED_VERIFICATION`; when Stage05 cannot obtain valid acceptance because the scoped evaluator/model/hardware blocker persists, use `INDEPENDENT_ACCEPTANCE_STATUS: BLOCKED` and `TASK_CLOSURE_STATUS: ACCEPTANCE_BLOCKED`, preserving the Stage04 snapshot. This is not an implementation or record-level veto and cannot be self-waived.
