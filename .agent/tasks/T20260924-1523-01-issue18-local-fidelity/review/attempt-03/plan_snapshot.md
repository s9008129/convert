# Issue #18 Local Meeting Record Architecture V2 — Plan Revision 4
...
Run focused synthetic suite and v4.7.4 regression set; full `pytest tests/ -q`, `bash scripts/check_docs.sh`, `git diff --check`. Run selected-claim end-to-end first and follow complete evidence chain. Then fresh Gemma and Qwen E2E. Complete C14 repeated candidate-profile sampling before final profile selection; report sample counts and median/worst/range/hard-fail count. Separately run blind 3+3 with frozen rubric and original unrounded Gemini baseline; diagnostic/profile-selection runs are excluded. Show per-output/per-dimension minimum, median, worst, range and hard-fail count. If evaluator/baseline missing, preserve all implementation/E2E evidence and record exact scoped quality blocker. Add held-out different-template/meeting evidence only if an approved privacy-safe source exists; never use private data in tracked artifacts.
...
|---|---|---|---|---|---|---|---|---|---|
| C1 selected causal claim source→facts→render→delivery | CORE | OUTCOME | HARD_CLEAN | NO | Inversion/omission/conflict is CORE FAIL; separate fixture defect | NO | NONE | NOT_RUN | NOT_ALLOWED |
| C2 synthetic A–I and structured ledger/patch contract | CORE | OUTCOME | HARD_CLEAN | NO | Changed contract failure is TASK_REGRESSION | NO | NONE | NOT_RUN | NOT_ALLOWED |
...
| C6 full `pytest tests/ -q` | SUPPORTING | REPOSITORY_HEALTH | HARD_CLEAN | YES | Baseline failure remains debt; new/worsened relevant signature is TASK_REGRESSION | NO | NONE | NOT_RUN | NOT_ALLOWED |
...
| C9 Gemma/Qwen fresh E2E | CORE | OUTCOME | HARD_CLEAN | NO | Selected-claim failure or hidden fallback is FAIL; missing model/backend is scoped environment BLOCKED | NO | NONE | NOT_RUN | NOT_ALLOWED |
| C10 blind 3+3 frozen rubric + zero major hard-fails | CORE | OUTCOME | HARD_CLEAN | NO | Per-model every output/dimension ≥ frozen Gemini median×0.80 and zero major failures; missing evaluator/baseline is scoped BLOCKED | NO | NONE | NOT_RUN | NOT_ALLOWED |
...
| C14 repeated candidate-profile sampling and selection | CORE | OUTCOME | HARD_CLEAN | NO | Per candidate N≥5 preferred/N=3 minimum with limitation recorded; compare median/worst/range/hard-fail count and select by the ordered fidelity-first priorities in W7; unavailable model/hardware is scoped environment BLOCKED | NO | NONE | NOT_RUN | NOT_ALLOWED |
