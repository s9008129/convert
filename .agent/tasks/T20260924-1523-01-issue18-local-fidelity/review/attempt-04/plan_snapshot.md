# Issue #18 Local Meeting Record Architecture V2 — Plan Revision 5
...
- WORKTREE_AT_PLANNING: contains Stage 04 V2 implementation and task artifacts; no unrelated owner changes observed
...
| R8 | Separate model-independent QualityPolicy from runtime profiles/capability detection for Qwen/Gemma; evaluate explicit initial Qwen/Gemma controls and Qwen extraction-temperature A/B candidates with repeated runs | CORE | Profile stability is required for reproducible quality. Capability-invalid parameters are explicitly downgraded/reported; no model-specific fidelity criteria. |
...
| C14 repeated candidate-profile sampling and selection | CORE | OUTCOME | HARD_CLEAN | YES | Live runtime exposes/validates candidate settings; per candidate N≥5 preferred/N=3 minimum with limitation; compare median/worst/range/hard-fail count and select by W7 priorities; missing evaluator is scoped quality BLOCKED and unavailable model/hardware scoped environment BLOCKED | NO | NONE | NOT_RUN | NOT_ALLOWED |
...
Profile-selection acceptance additionally requires repeated candidate-profile runs at the counts and statistics in W7/C14; these diagnostic runs never count as blind 3+3 samples.
