# Issue #18 Local Meeting Record Architecture V2 — Plan Revision 8
...
| C9 Gemma/Qwen fresh E2E | CORE | OUTCOME | HARD_CLEAN | NO | CORE acceptance input and required verification item, not a per-record gate. Selected-claim failure or hidden fallback is FAIL; model/hardware unavailable after safe load/availability check is `CHECK_RESULT: BLOCKED` (`ENVIRONMENT`). Stage04 maps a known blocker to `CORE_ACCEPTANCE_STATUS: BLOCKED`, verification `INCOMPLETE` when other required evidence exists (otherwise phase `BLOCKED` only if no valid conclusion), independent acceptance `PENDING`, closure `CORE_ACCEPTANCE_BLOCKED`. If Stage05 cannot obtain a valid E2E result, set independent acceptance `BLOCKED`, preserve the Stage04 snapshot, closure `ACCEPTANCE_BLOCKED` | NO | NONE | NOT_RUN | NOT_ALLOWED |
| C10 blind 3+3 frozen rubric + zero major hard-fails | CORE | OUTCOME | HARD_CLEAN | YES | CORE acceptance input and required verification item, not a per-record gate. Per-model every output/dimension ≥ frozen Gemini median×0.80 and zero major failures; missing evaluator/rubric/original unrounded baseline is `CHECK_RESULT: BLOCKED` (`AUTHORITY`) with `QUALITY_ACCEPTANCE_BLOCKED_BY_MISSING_FROZEN_EVALUATOR`, never a guessed PASS/FAIL or waiver. Stage04 maps known blocker to core acceptance `BLOCKED`, verification `INCOMPLETE` when other required evidence exists (otherwise phase `BLOCKED` only if no valid conclusion), independent acceptance `PENDING`, closure `CORE_ACCEPTANCE_BLOCKED`. If Stage05 cannot obtain valid acceptance, independent acceptance `BLOCKED`, preserve Stage04 snapshot, closure `ACCEPTANCE_BLOCKED` | NO | NONE | NOT_RUN | NOT_ALLOWED |
