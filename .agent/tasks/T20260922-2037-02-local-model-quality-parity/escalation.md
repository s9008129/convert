...
## 追加（E3 獨立驗收後新增；原節保留不重寫）

### STOP-5：B1 歸屬絆索在段落邊界**必然假陽性**（已確認，修補中）

- 事實：E3 場 B1 共 14 個判定**全部**為假陽性——標註時間戳同時是「前一位發言者段落結尾」與
  「標註所指發言者段落起點」，查找雙端含等號且由檔序取首命中 → 永遠判給前一段。
  P4-D 的標註吸附**刻意**把時間戳吸附到真實段落起點，使此 bug 幾乎必然觸發。
- 影響：cap=3 筆假問題被送進每一輪補強提示詞，模型不可能修好 → 是「輪數 >1」的**共因**，
  因此輪數未達不能單獨歸因模型品質。
- 處置：同缺陷類別（假陽性）收尾修補，附真錯誤負向對照；若審查認為需新語意決策則回退並改列 planner。

### STOP-6：P4-A 數字對帳器子字串假命中（已確認，修補中）

- 事實：`600` 被判「已涵蓋」來自紀錄的 `13,600`；`15` 的命中來自紀錄清單編號 `15.`。
  逐字稿有「發 600塊」「少了 15%」，最終紀錄兩者皆 0 命中 → **真缺口被漏檢**、補強因此未提出問題。
- 影響：`cov_missing_number=0` 不成立；§9.3 CORE 的「數字逐筆命中」在 E3 僅 `800／17／13600` 成立。
- 處置：數字命中改要求數字邊界（保留千分位等價與 `600元`／`15%` 正常命中），附不得過嚴的負向案例。

### STOP-7：釘版量尺的探針群組可「形式命中」（**不得回溯修改，需 planner 決策**）

- 事實：`F024` 以 probe 群組 `['600','便當']` 在 `13,600` 內命中；`F010` 以 `['委任','40','25']` 命中但紀錄無 `15%`。
  → 清單 `number 4/4＝1.0` 被高估；人工校正估算 `coverage_all ≈0.7612`（`[INFERRED]`）。
- 依 plan §9.3 F1①「P4-A 驗收一律用釘版量尺、P4-G 新尺不得回溯」→ **本波不修改量尺輸出**，
  僅登錄限制；是否以新尺（`coverage-2.x`）重測並更新結論，屬 planner 決策。
