# P7-C 優化規劃：地端萃取與生成遵循度（模型無關）— rev9
...
1. Stage 01 escalation-replan rev9 → **Stage 02 獨立複審 attempt-09（fresh context，rev9 hash 專屬）**→ Stage 03 handoff（先 archive 當前 handoff）→ fresh Stage 04。保留前次 Stage 04 執行／escalation 歷史與 GEMMA-E1 原始資料；新 Stage 04 依 rev9 明確重整並驗證，不回寫或假稱舊結果。
...
5. **E2E clean-worktree 執行（回應 rev4 escalation；決策性 preflight）**：
   ...
   - **Apple ASR helper preflight（rev9）**：在 detached worktree 內執行既有 `swift build -c release --package-path apple_speech_cli`；確認 `apple_speech_cli/.build/release/apple-speech-cli` 存在且 `--help` 可啟動。確認 `.build/` 由 package `.gitignore` 忽略，建置後 porcelain 仍空、HEAD 不變。Package.swift 無外部 package dependencies。若 Swift/build/helper/clean check 失敗，設定 `E2E-ASR-HELPER-PREFLIGHT: BLOCKED`（ENVIRONMENT_FAILURE）、CORE acceptance `BLOCKED`；不要載入模型或啟動該輪 E2E，不複製其他 checkout 的 binary，也不修改 runner。
...
| `LOCAL-CONTRACTS` | NFR-相容：新開關（含 C2a fact coverage / C2c adherence 獨立 flags）、C1c 超限原子失敗、C2b local-only 輪數／budget、off/no-log、後處理順序不變式 | CORE | MUST_NOT_BREAK | HARD_CLEAN | NO | 任一核准語意違反、超限仍有 provider 呼叫／部分會議摘要、C2 flag 關閉仍增加 fact issues/metrics 或 adherence prompt、雲端輪數／prompt／輸出受 local-only 設定影響，或 budget 停止後未保留最新有效摘要＝TASK_REGRESSION；預期逐字稿-only＋摘要失敗警告不算部分摘要。 |
| `BYTE-ROLLBACK` | NFR-回退：依 §6 全部新旋鈕關閉、保留舊 coverage 預設，按 §8 step 3 固定合成 fixture／mock harness，與完整基準 commit `ab2528b718e8c91557fd72db174b0f9bb78d3a68` 比較 C2a、C2c、local／cloud prompt、coverage issue／metrics、最終輸出各 UTF-8 bytes；Stage04 保存 hash-only 證據 | CORE | MUST_NOT_BREAK | HARD_CLEAN | YES | 任一 expected/actual byte hash 不同＝TASK_REGRESSION；固定 commit、fixture、可比輸出／hash 或證據缺失＝此 check BLOCKED；禁止以真實模型輸出或原始敏感內容補作比較。 |
...
| `E2E-COMMIT-PROVENANCE` | 每場前後 detached worktree `HEAD` 等於產品測試 commit、`git status --porcelain` 為空，health expected/actual build revision 相符；完整 runner artifacts/runtime 僅存 gitignored cache；task evidence 符合固定 redaction allowlist、repo-relative cache key 與原件 SHA-256 | SUPPORTING | MUST_NOT_BREAK | HARD_CLEAN | NO | 缺 SHA／dirty worktree／health revision mismatch／cache 分層錯誤／task evidence 含 raw payload 或無原件 hash＝該項 BLOCKED、受影響的 E2E-dependent CORE checks BLOCKED、CORE acceptance=`BLOCKED`、closure=`CORE_ACCEPTANCE_BLOCKED`；implementation 狀態保留。不得計 CORE PASS 或稱 implementation blocker。這是 E2E decision-validity 與隱私分層義務。 |
| `E2E-ASR-HELPER-PREFLIGHT` | E2E execution prerequisite：在固定 detached commit 本地建置 helper、可執行且 build artifacts 被忽略，前後 HEAD／porcelain 不變 | SUPPORTING | DIAGNOSTIC | HARD_CLEAN | NO | Swift build、helper smoke check 或 clean 狀態失敗＝具名環境 BLOCKED，該場不啟動／不計分、CORE acceptance BLOCKED、implementation 狀態保留；這是取得有效真實模型 E2E 證據的必要工具前置，不是產品 implementation gate。 |
...
`E2E-COMMIT-PROVENANCE` 為 SUPPORTING／MUST_NOT_BREAK／HARD_CLEAN：它不評產品品質，也不否決 implementation；但 E2E runner 本身將 clean HEAD 設為 decision-validity precondition，因此無法證明固定 commit、clean 狀態與 artifact/runtime 隔離時，該場輸出不構成有效 CORE 證據，CORE acceptance 不能據此 PASS。Detached worktree 把失敗範圍限於 E2E 證據，不改 runner gate、產品語意或主工作樹任務證據。

E2E 原始資料依 `security-privacy.md` 僅寫入本機 gitignored、task-scoped cache；`--artifacts-dir` 也按實際實作當 raw payload 處理，即使 CLI 文件稱之為 redacted。只有固定 allowlist 的 derivative、repo-relative `CACHE_KEY` 及雜湊留在 task evidence；不得出現絕對本機路徑、完整 request/response、自由文字 prompt、輸入音檔 basename、transcript 或 DOCX。Stage 05 必須 read-only 對照本機原件，引用僅限驗證 CORE-A／ANTI-GAMING 所必需的最短去識別 excerpt。
