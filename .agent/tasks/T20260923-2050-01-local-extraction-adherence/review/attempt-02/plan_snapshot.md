# P7-C 優化規劃：地端萃取與生成遵循度（模型無關）— rev2
...
- **C1b 區塊細化（值已釘死）**：E2E 使用 **`LOCAL_LLM_EXTRACTION_CHUNK_CEILING_TOKENS=4500`**。
  ...
  - 預註冊預測（含 FALSIFIER）逐條引用 `evidence/chunk-replay-p7c/README.md §3`，並於每場開跑前登錄。
...
- **C1c 呼叫數護欄（新旋鈕）**：`LOCAL_LLM_EXTRACTION_CALL_BUDGET`
  ...
  - 逾限行為：停止加塊、記 `skipped_reason=extraction_call_budget_exceeded`（observation-only log）。
...
| **CORE-E** 成本（取代相對門檻） | ①**呼叫數**（確定性）：萃取呼叫數／補強輪／logical_generations；②**牆鐘**（`run_summary.json` `started_at`→`finished_at`，**非** pipeline total） | `backend.log` metrics＋`run_summary.json` | ①萃取呼叫 ≤ 實際塊數且 ≤ `CALL_BUDGET`(=4)；補強輪 ≤3；②牆鐘 **gemma ≤2,750 s**（＝P7-B 2,378.5＋340.6 整份萃取）、**qwen ≤2,400 s**（＝1,814.9＋360 補強輪＋150 萃取） |
...
- 新開關全部登錄 `.env.example`；skip 路徑記 `skipped_reason`（observation-only）。
- Windows／Ollama 實機仍 `[UNVERIFIED]`，Stage 05 對「守衛是否被評估／為何被跳過」做顯式斷言。

## 8. 驗證計畫（Stage 04 → Stage 05）

1. Stage 01（本檔）→ **Stage 02 獨立審查**（必，attempt-02 fresh context）→ Stage 03 handoff → Stage 04 實作。
2. 每個 CORE 項目先做**零呼叫驗證**（離線重播／單元測試），再做 E2E。
3. **E2E 前置（新，I12）**：實作完成且單元測試綠 ⇒ **先 commit（僅產品碼＋測試）** ⇒ 才啟動 E2E。
   理由：runner 對 dirty worktree **fail-closed**（P7-A 已因此白跑一場 `e2e/attempt-P7A-gemma31b-e7b`）。
4. Stage 05 獨立驗收：CORE-A／B／C／D／E／F 逐條、DOCX、反 gaming 抽驗、成本對帳、殘餘風險。
5. 產出 `result.md`；誠實登錄未達項，不得以觀察值替代阻斷條文。

## 9. 預算與切片

- 4 場 E2E ≈ 2 小時 20 分（gemma ≈40 分／場、qwen ≈30 分／場；C1b 後 gemma 側每場 +約 150–340 s）
  ＋離線驗證與實作＋S3／S5（資源允許）。
...
|---|---|---|
| 證據 | 已量測：尾半段單次呼叫 **6/7 命中（含 F044）**、**150.5 s** | 零呼叫重播（3 塊、F044 落 4,413 塊）＋P7-B 已證「分塊取代大呼叫」 |
| 成本 | +1 次呼叫（+150.5 s，**已量測**） | +1 次呼叫（每塊 ≈150–340 s，區間由實測推得） |
| 覆蓋範圍 | **只有尾段**（中段漏寫無解） | **整份逐字稿**（每塊都有獨立呼叫） |
| 模型無關 | 是 | 是 |
| 風險 | 需決定「尾段」定義（時間／比例）＝新啟發式；單次視窗是**機率性**（兩次各 6/7 但命中集合不同） | 塊數隨逐字稿長度成長 ⇒ 成本需護欄（C1c） |
| 決策 | **備援**：若 C1b 之後尾段仍 <5/7（FALSIFIER 成立），才啟用 A（+150.5 s，成本已量測） | **主方案**（覆蓋範圍與模型無關性勝出） |
