...
## 0.10 rev12 變更紀錄（回應最新使用者目標與 attempt-03）
...
## 1. Goal Contract（白話；rev12 生效）

使用者要的是：**Gemma 4 31B 與 Qwen 3.8 27B 產生的會議紀錄，各自與專案設定的 Gemini 3.5 Flash-lite 品質差距都嚴格小於 10%。** 目前 `backend/core/config.py` 的 `GEMINI_MODEL` 指向 `gemini-3.5-flash-lite`，`summarize(mode=cloud)` 進 `_summarize_with_gemini`。公平比較不是「同一 prompt 的裸模型競賽」：比較各自現行產品摘要路徑在相同文字輸入下的輸出；兩路提示、萃取與後處理依產品設計可能不同，必須記錄但不可混淆成同 prompt。

P7-B 已顯示兩個模型的失效位置不同：Gemma 尾段重要事實仍漏；Qwen 密度改善，但曾漏掉已在萃取筆記中的事實並增加耗時。P7-C 已實作觀測與控制能力，但它此前的兩場規劃與「不宣稱 parity」不再是此任務的驗收條件。Rev12 先取得公平配對品質證據，再用完整 E2E 驗證交付整合。

**核心路徑**：固定同一 canonical transcript → cloud/Gemma/Qwen 現行產品摘要路徑各 3 次 → 逐份以固定人工語意 rubric 計分 → Gemma、Qwen 每一份輸出都對 cloud 中位數達到各維度嚴格 `<10%` 短差且無重大捏造／矛盾 → 各自一場完整 audio-to-DOCX E2E（只驗交付整合）→ Stage 05 獨立驗收。任何核心品質維度失敗都不可由更好的密度、字數或另一維度平均補償；若基線有效但未達標，帶著具體失敗事實重新規劃修正，不得把「量到差距」冒充「完成目標」。

### rev11 歷史量測（僅供根因與趨勢參考，非 rev12 acceptance）
...
## 3. P7-C 既有改善設計（rev11 已落地；不是 rev12 parity 結果）
...
## 4. rev11 SUPPORTING／BEST_EFFORT（歷史非阻斷項；rev12 不納入 parity 門檻）
...
**路徑、樣本與公平性**：以 `scripts/e2e/rerun_local_summarize.py` 呼叫正式摘要服務；`cloud` 的有效模型必須核對為 `gemini-3.5-flash-lite`，local 模型 key 必須分別為 `gemma-4-31b-it-mlx` 與 `qwen3.8-27b-splash`（Qwen 3.8 **27B**）。local 初始 benchmark 固定使用 rev11 預註冊的 P7-C profile：`LOCAL_LLM_EXTRACTION_CHUNK_CEILING_TOKENS=4500`、`LOCAL_LLM_EXTRACTION_CALL_BUDGET=4`、fact/adherence flags=`true`、local refinement rounds/call budget=`3`、C3 專名正規化=`false`、既有 coverage mode/categories 保持 `enforce`／`topic,decision,number,date`；雲端只用其現行設定。因 config defaults 中兩個新 flags 預設 `false`，此 profile **不是目前預設 runtime 的等價宣稱**；若它通過品質量尺，必須在經審核的後續 implementation 中使產品實際設定與被評 profile 一致，再以最終有效設定重驗，否則不可宣稱達標。
...
**逐份盲評的 CORE 維度**（Stage 05 在本地 raw outputs 上獨立評分；使用固定 rubric，盡可能遮蔽模型名稱；不把原始輸出複製到任務 artifact）：

| CORE 維度 | 可重算定義 | 硬性錯誤／限制 |
|---|---|---|
| `COMPLETENESS` 核心事實完整性 | 28 條 core facts 逐條人工對照 canonical transcript，分 `HIT=1`、`PARTIAL=0.5`、`MISS=0`、`FABRICATED=0`；分數＝加總／28。現有 `coverage-1.0.0` 字面命中只作旁證，因跨行誤命中與同義表達漏命中已被實證。 | 不得將 checklist token 餵入模型。`FABRICATED` 同時觸發下列 fidelity hard fail。 |
| `FAITHFULNESS` 事實／語意保真 | 逐份核對所有實質主張；分數＝有逐字稿依據且未被反駁的主張數／全部實質主張數；另列否定、條件、數字、日期、主體／歸屬是否被改寫。 | 任一可確認的重大捏造、矛盾、數字或歸屬錯誤＝該模型 CORE `FAIL`，不可由完整度或文筆抵銷。 |
| `TRACEABILITY` 來源可追溯 | 逐份核對實質事實主張是否能定位至 canonical transcript 的來源片段／時間標籤；分數＝可定位主張數／全部實質主張數。 `00:00:00` 只有在來源確實位於該時間才算可追溯，不接受「全標零」虛高。 | 證據缺失或來源 transcript hash 不符＝`BLOCKED`；錯誤來源／偽標籤按 faithfulness 評分並揭露。 |
| `USABILITY` 結構與可用性 | 固定 0–100 錨定 rubric（結構清楚、議題不混淆、決定／待辦／責任與期限可辨、可讀性、非冗贅五面向各 0/25/50/75/100；維度分數為五項算術平均），三份輸出均盲評。 | rubric 必須在讀取模型輸出前凍結；item count、平均字數、near-duplicate 不能替代此分數。 |

**嚴格 `<10%` 的操作定義**：對每個高分越好維度 `d`，先取 3 份有效 Gemini 分數的中位數 `C_d`。對每份 local 輸出 `L_d` 計 `gap=max(0, 1 - L_d/C_d)`；保留未四捨五入分數，僅當 `gap < 0.10` 才通過（等於 10% 即 FAIL）。若 `C_d <= 0`、baseline 呼叫／model identity 無效或分數無效，該模型 parity 結論為 scoped `BLOCKED`，不得改用 P4 歷史數字、加 epsilon 或換分母。Gemma 與 Qwen 分開計算；**每個模型 3/3 份、每個 CORE 維度皆通過**才使該模型 CORE PASS。禁止先對三次 local 結果取中位數再掩蓋單次大幅退化，也禁止跨維度總平均。
...
### rev11 歷史 P7-C 門檻（只供根因／趨勢參考；不是 rev12 acceptance）
...
1. Stage 01 更新同一任務 `PLAN_REVISION=12` → **Stage 02 fresh attempt-12**（須記錄本 rev12 snapshot/hash；舊 attempt-11 核准不適用）→ Stage 03 archive 舊 handoff 並編譯 rev12 → fresh Stage 04。核對 handoff 與 plan revision/hash 一致後才可執行。
2. Stage 04 在 `execution.md` append-only 補記 attempt-03 的確切終態：helper build/probe 已過，但 wrapper 預建 runner-owned artifact directory、runner `exist_ok=False` 因而 exit 1；backend／upload／摘要推論均未開始，故不是模型品質 FAIL。不得回寫舊 attempt、刪除或重用其 cache。
3. 開跑前只讀核對 HEAD/工作樹、canonical transcript/checklist/model identity 與 settings。目標 transcript 必須逐 byte SHA 相符；雲端實際 model/provider 不是 `gemini-3.5-flash-lite`、local model key 不是 §5 指定模型、模板/settings 無法確定或資料缺失時，停止並作 scoped BLOCKED，不可自動 fallback 到其他模型／舊輸出。
4. 以 §5 固定 protocol 進行 9 次同 transcript 摘要呼叫；建議 round-robin `cloud → gemma → qwen` ×3 以減少時間／服務漂移偏差。每次輸出、工具 stdout/stderr、service log 都寫入新且 gitignored 的 raw cache；每個 `--out` 唯一，不預建會由工具建立的目錄。task artifact 只留固定 allowlist 的 alias、model family、hash、metric 分數／IDs、有效性與耗時；不得貼出逐字稿或紀錄正文。
5. 若有效比較顯示任一 local model 任一 CORE 維度 gap `>=10%` 或有 fidelity hard fail，CORE=`FAIL`，先寫 evidence-backed gap diagnosis 並進 Stage 01 `ESCALATION_REPLAN`，不為了完成性先跑 supporting E2E。模型品質修正須有新核准 revision/handoff；不可在本 rev12 benchmark wave 偷改 prompt／settings／程式後重用舊 cloud baseline。
6. 僅當 Gemma、Qwen 都達到 §5 3/3 × 全維度條件後，才各執行一場 full audio→DOCX E2E，驗證正式交付鏈。用全新 ignored cache keys；`runner-output-raw/`、runtime 目錄須起初不存在並交由 runner 建立，絕不可預建 runner-owned artifacts directory。此兩場只驗整合、不混入 parity 分數。Stage 05 對 parity 評分及 E2E 交付作 fresh read-only 獨立驗收。

### rev11 P7-C 實作波程序（歷史；已執行，不是 rev12 的當前 runbook）

以下 rev11 步驟記錄既有 P7-C controls 的開發／回退證據程序，留作 provenance 與將來修正時的參照；不得取代 rev12 §5 的品質目標，也不得因其中寫有「CORE-A–F」或四場就當成 parity PASS。rev12 若要改現行語意，必須先由 Stage 01 針對新證據重規劃。
...
|---|---|---|---|---|---|---|
| `PARITY-COMPLETENESS` | REQ-品質：Gemma/Qwen 核心事實完整度相對 Gemini | CORE | OUTCOME | HARD_CLEAN | NO | 任一 local 3 份輸出 gap `>=10%`＝FAIL；輸入／評分無效＝BLOCKED；依 §5 不可改用字面 coverage 取代語意逐條評分。 |
| `PARITY-FAITHFULNESS` | REQ-品質：主張有據、無重大捏造／矛盾 | CORE | OUTCOME | HARD_CLEAN | NO | 任一 local 重大捏造、關鍵數字／歸屬錯誤或語意反轉＝FAIL；除此之外任一輸出 gap `>=10%` 亦 FAIL；逐字稿／輸出證據缺失＝BLOCKED。 |
| `PARITY-TRACEABILITY` | REQ-品質：重要主張能追溯到同一逐字稿來源 | CORE | OUTCOME | HARD_CLEAN | NO | 任一 local 輸出相對 cloud 中位數 gap `>=10%` 或出現偽來源標記＝FAIL；來源證據無效＝BLOCKED。 |
| `PARITY-USABILITY` | REQ-品質：固定盲評 rubric 下的結構與可用性 | CORE | OUTCOME | HARD_CLEAN | NO | 任一 local 輸出相對 cloud 中位數 gap `>=10%`＝FAIL；rubric 未預先凍結或評分缺失＝BLOCKED。 |
| `PARITY-PROVENANCE` | 決策有效性：canonical input/checklist、產品 commit、實際 cloud/local model key、量測與原始輸出 hash 均符合 §5 | CORE | MUST_NOT_BREAK | HARD_CLEAN | NO | 任一 hash/model/commit/config 不符或 raw evidence 無法獨立重核＝BLOCKED；不得把舊 C5 或不一致 transcript 補入樣本。 |
| `PROFILE-PERSISTENCE` | REQ-交付：最終實際產品設定與通過 parity 的 local profile 相同 | CORE | MUST_NOT_BREAK | HARD_CLEAN | NO | 只有臨時 shell/.env 覆寫達標、產品實際設定未落地或未以最終設定重驗＝FAIL／NOT_RUN，不能宣稱目標達成。 |
...
| `LOCAL-CONTRACTS` | NFR-相容：local/cloud 隔離、逾限原子失敗、local-only 輪數與 budget、off/no-log、既有後處理與 fallback 語意 | SUPPORTING | MUST_NOT_BREAK | HARD_CLEAN | NO | 任一已核准語意違反、雲端受 local-only 設定影響、超限仍有 provider 呼叫／部分紀錄或失敗降級不符＝TASK_REGRESSION。 |
...
| `STAGE05-INDEPENDENT` | REQ-獨立驗收：重算逐份 parity 分數並核對兩場 audio-to-DOCX integration | CORE | OUTCOME | HARD_CLEAN | NO | 獨立驗收依證據 PASS／FAIL；外部輸入／工具／環境造成無效結論＝scoped BLOCKED，不能抹除 Stage 04 已證狀態。 |
...
- 核心先跑 9 份固定 transcript 摘要（cloud/Gemma/Qwen 各 3 份）；這與舊 full E2E 每場成本不同，不能套用 §5 rev11 的 2,750／2,650 秒為此工作保證。每次記錄實際 elapsed、呼叫數與模型設定；遇到服務正式 timeout 即保留失敗並分類，不盲目重試或選擇性補最好結果。
- 只有 9 份對照達標後，才跑 2 場真正 local audio-to-DOCX integration。若 parity 失敗，先 replan 修核心缺口，暫不消耗 E2E 成本；修正後使用新的 plan revision、raw cache keys 與獨立驗收 attempt。
...
- `[VERIFIED]` 目前兩個 P7-C 新 flags 預設關閉；其 profile 即使在臨時 benchmark 環境達標，也不能代表使用者目前預設設定。需要經後續 review 把設定落在產品實際設定，並以該 profile 重驗。
