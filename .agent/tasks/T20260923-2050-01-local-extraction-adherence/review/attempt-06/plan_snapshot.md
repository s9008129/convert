# P7-C 優化規劃：地端萃取與生成遵循度（模型無關）— rev6
...
1. Stage 01 review-revision rev6 → **Stage 02 獨立複審 attempt-06（fresh context，rev6 hash 專屬）**→ Stage 03 handoff（先 archive rev4 handoff）→ fresh Stage 04。
2. **產品碼變更前取得完整全庫基線**：`DATA_DIR=/tmp/test_scratch uv run --frozen pytest tests/ -q`；保存完整命令、結束碼、摘要及必要 log。曾被中斷的 pytest 不算基線。若不能取得有效基線，依 §8.1 的 `FULL-SUITE` 標記，不得假設為乾淨。
...
5. **E2E clean-worktree 執行（回應 rev4 escalation；決策性 preflight）**：
   ...
   - 四場固定 Gemma 4 31B ×2、Qwen 3.8 27B ×2；每場前後記錄 detached worktree HEAD／porcelain 和 SHA。產出若落入不合規路徑、task evidence 含未 redacted raw payload、HEAD 改變或 worktree 出現非空 status，該場不得計入 E2E PASS；立即停後續 E2E 並記錄 scoped blocker／replan，不修改 runner、ignore/exclude 或預註冊門檻。
