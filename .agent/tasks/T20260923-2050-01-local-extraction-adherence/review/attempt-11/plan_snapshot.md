# P7-C 優化規劃：地端萃取與生成遵循度（模型無關）— rev11
...
1. Stage 01 escalation-replan rev11 → **Stage 02 獨立複審 attempt-11（fresh context，rev11 hash 專屬）**→ Stage 03 handoff（先 archive 當前 handoff）→ fresh Stage 04。保留前次 Stage 04 執行／escalation 歷史、attempt-02 的 historical BLOCKED 結果與 GEMMA-E1 原始資料；新 Stage 04 依 rev11 明確重整並驗證，不回寫或假稱舊結果。
...
| `E2E-COMMIT-PROVENANCE` | 每場前後由實際執行 repo context 證明為新 detached worktree、`HEAD` 等於產品測試 commit、runner script root 與 checkout 相符、`git status --porcelain` 為空，health expected/actual revision 相符；完整 raw artifacts/runtime 僅存 gitignored cache；task evidence 僅 allowlist bool/hash/cache key | SUPPORTING | MUST_NOT_BREAK | HARD_CLEAN | NO | 缺 SHA/root-match/detached proof／dirty worktree／health revision mismatch／cache 分層錯誤／task evidence 含 raw payload 或無原件 hash＝該項 BLOCKED，受影響 CORE evidence BLOCKED；implementation 狀態保留。父 repo 無法列出 subagent 隔離 worktree 不單獨判失敗，執行 context 缺可核實證據才阻斷。 |
| `E2E-ASR-HELPER-PREFLIGHT` | 固定 commit detached worktree build helper；`probe --locale zh-TW` 的 raw 只在 ignored cache，驗 exit=0 且 locale_supported/transcriber_is_available/locale_installed 為 true | SUPPORTING | DIAGNOSTIC | HARD_CLEAN | NO | build/probe/locale 可用性失敗＝具名 ENVIRONMENT_FAILURE/BLOCKED，不載入模型或啟動 runner；locale 未安裝＝AUTHORITY_REQUIRED/BLOCKED，先請使用者授權 Apple asset installation；不把 rev10 `--help` exit5 誤稱 runtime failure。兩類只阻斷 E2E/CORE acceptance，保留 implementation。 |
