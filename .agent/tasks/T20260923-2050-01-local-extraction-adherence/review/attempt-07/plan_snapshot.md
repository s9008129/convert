# P7-C 優化規劃：地端萃取與生成遵循度（模型無關）— rev7
...
1. Stage 01 escalation-replan rev7 → **Stage 02 獨立複審 attempt-07（fresh context，rev7 hash 專屬）**→ Stage 03 handoff（先 archive 當前 rev6 handoff）→ fresh Stage 04。保留 Stage04 rev6 執行／escalation 歷史與目前未提交產品/test WIP；新 Stage04 依 rev7 明確重整並驗證，不回寫或假稱其舊結果。
...
| `BYTE-ROLLBACK` | NFR-回退：依 §6 全部新旋鈕關閉、保留舊 coverage mode/categories 預設時逐位元回到本波前產品行為 | CORE | MUST_NOT_BREAK | HARD_CLEAN | YES | 與固定 P7-B／ab2528b 的 prompt、metrics、輸出 bytes 不一致＝TASK_REGRESSION；基準檔或可比輸出缺失＝BLOCKED。 |
