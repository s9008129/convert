# P7-C 優化規劃：地端萃取與生成遵循度（模型無關）— rev8
...
1. Stage 01 escalation-replan rev8 → **Stage 02 獨立複審 attempt-08（fresh context，rev8 hash 專屬）**→ Stage 03 handoff（先 archive 當前 rev6 handoff）→ fresh Stage 04。保留 Stage04 rev6 執行／escalation 歷史與目前未提交產品/test WIP；新 Stage04 依 rev8 明確重整並驗證，不回寫或假稱其舊結果。
...
5. **E2E clean-worktree 執行（回應 rev4 escalation；決策性 preflight）**：
   ...
   - 四個穩定 run alias 固定為 `GEMMA-E1`／`GEMMA-E2`／`QWEN-E1`／`QWEN-E2`，cache keys 相應固定為 repo-relative `data/cache/e2e/p7c-gemma-e1`、`data/cache/e2e/p7c-gemma-e2`、`data/cache/e2e/p7c-qwen-e1`、`data/cache/e2e/p7c-qwen-e2`；每場 `runner-output-raw/` 與 `runtime-raw/` 均各自唯一且不存在。
