...
## 1. Goal Contract（白話；rev13 生效）

使用者要的是：**正式產品地端路徑中的 Gemma 4 31B 與 Qwen 3.8 27B，各自與專案雲端 Gemini 3.5 Flash-lite 的會議紀錄品質差距都嚴格小於 10%。** 目前 `backend/core/config.py` 的 `GEMINI_MODEL` 指向 `gemini-3.5-flash-lite`，`summarize(mode=cloud)` 進 `_summarize_with_gemini`。比較的是各自產品摘要路徑在相同文字輸入下的結果，不是同一 prompt 的裸模型比賽；兩路提示、萃取及後處理差異要記錄。
...
**核心路徑**：plan 審核核准 → rubric 凍結與 hash → 候選 3×3 + blind score → 候選未過則有證據地重規劃、不得先改預設或跑 supporting E2E；候選通過則只修改核准的 local-only defaults／範例設定 → 驗證 `.env`／process environment precedence 與雲端不變 → 新產品 commit 上再做全新 3×3 + 獨立 blind scoring → Gemma/Qwen 各一場 supporting audio-to-DOCX E2E → Stage 05 read-only acceptance。最終每個模型的每份輸出都須對 Gemini 3 份中位數在四維各自嚴格 `<10%`，並且無重大捏造／矛盾。有效失敗不能被密度、字數或另一維度抵銷。
...
| `TRACEABILITY` 來源可追溯 | 對相同原子事實主張，逐一核對可否定位至正確 canonical transcript 段落／時間標籤；分數＝正確 source anchor 主張數／全部可追溯事實主張數×100。 | 缺失證據／hash 不符＝`BLOCKED`；錯誤來源／偽標籤不計成功。分母為 0 時 invalid/BLOCKED，不接受全標 `00:00:00` 虛高。 |
...
### rev13 當前執行波（候選門檻 → 持久化 → 正式品質驗收）

1. Stage 01 更新同一任務 `PLAN_REVISION=13` → **Stage 02 fresh attempt-13**（對當前 Plan snapshot/hash 出具 gate）→ Stage 03 archive 舊 handoff 並編譯 rev13 → fresh Stage 04。核對 handoff 與 Plan revision/hash 一致後才可執行；rev12 的 attempt-12 是拒絕修訂，不可沿用其 approval/handoff。
