...
- 實測效果（本次輸出 vs baseline）：
  ...
  - **反向證據（必須誠實記錄）**：最終紀錄本身**完全沒有**「發言者N」字樣（`二、` 47 條、決議彙整表 63 筆資料列皆無；全檔只有 `一、` 有 5 行（共 10 處）零星引註如「發言者3建議…」）。也就是說：**標籤只存在於逐字稿**，紀錄的裁示歸屬目前只能靠人工比對逐字稿推斷。
...
| 2 | ~~文件 §9 補 E2E 證據~~ **已完成（本輪接手複核）** | 產物：`doc/規格與設計/發言者分離與主席裁示-研究與設計.md` §9（含 task id、標籤樣本、C1–C4 證據、輸出檔名與大小） | ✔ §9 已具備；細節見 §11 |
| 3 | ~~文件一致性複核~~ **已完成（本輪接手複核）** | 唯讀子代理對照 §2/§3/§5/§6 與程式碼（預設值、fp32、逾時/守門/快取、provider 中立） | ✔ 核心無矛盾；另修 3 處精確度落差（§7.0／§9.4／§9.6②）＋R-9 狀態；細節見 §11 |
...
| 5 | ~~最終回報使用者~~ **已完成（本輪接手）** | primary outcome → 閉環 → 殘留風險（見 §9） | ✔ 2026-09-13 22:2x 已於對話回報（含勘誤與新方向登載） |
...
| 7 | （建議另開新任務）ASR 語意／錯字校正移植 | 使用者另一專案完成校正後，把關鍵修正邏輯移植回本專案（可能落點：ASR 後處理層或提示詞層）；**不屬本 GOAL 範圍**；校正引擎**跟隨使用者選擇的處理模式**（地端模式→地端模型；雲端模式→雲端模型） | 移植後以同一支音檔重跑 E2E 回歸，比對標籤／裁示歸屬／輸出檔差異 |
...
7. **語意／錯字校正引擎**（使用者 2026-09-13 補充修正，取代同日稍早「一律用雲端」指示）：校正引擎**跟隨使用者選擇的處理模式**——地端模式用**地端模型**、雲端模式用**雲端模型**。本任務不執行校正；移植時依此方向落地。
...
4. **ASR 文字品質（已向使用者完整說明；結論：不阻擋本任務）**：
   ...
   - **校正引擎（使用者 2026-09-13 補充修正）**：語意／錯字校正**跟隨使用者選擇的處理模式**——地端模式用**地端模型**（本機 LM Studio 目前未開，走 fail-soft 跳過屬預期）、雲端模式用**雲端模型**（本任務之雲端＝Ollama Cloud `deepseek-v4.1-flash`）。
...
## 11. 本輪接手紀錄（2026-09-13 22:1x–22:4x；收尾與複核）

- 角色：新對話主線（收尾／整合）；**未改產品程式碼、未重啟服務、未 commit**。
- 並行唯讀子代理：① Stage 05 報告 vs 現況複核 ② 研究文件 ↔ 程式碼一致性稽核 ③ 本輪 diff 複核。
- 複核結論：C1–C4 證據可重跑且與報告一致；文件與程式碼核心無矛盾（預設值／fp32／逾時守門／快取／provider 中立逐項核對）。
- **勘誤（append-only，見 `e2e/attempt-01/e2e_report.md` 附錄 A）**：紀錄檔 `發言者N` 引註＝5 行/共 10 處（原記 5 處）；決議彙整表＝63 筆資料列（原記 65 列）；雲端呼叫＝7 次（原記 5 次）。gate 與 C1–C4 判定不變。
- 文件修正：§7.0 指令補 `DATA_DIR=./data`（macOS 直跑必需）、§9.4 補正式檔路徑、§9.6② CI 機制更正（實為 pytest `tests/test_system_prompt_validation.py`）、R-9 由「E2E 進行中」改為「已完成」、§5 末補 E2E 結果。
- 測試（本輪實跑）：`DATA_DIR=./data .venv/bin/python -m pytest tests/ -q` → **784 passed, 2 skipped, 3 warnings in 10.00s**；聚焦 `tests/test_diarization_failsoft.py tests/test_speaker_transcript.py` → 42 passed。
