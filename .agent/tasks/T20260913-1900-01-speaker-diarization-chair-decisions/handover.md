...
- 實測效果（本次輸出 vs baseline）：
  ...
  - 主席段落擴充為 13 個主題＋65 列決議彙整表（baseline 為單一「（待確認）」）。
...
2. **C2 屬部分達成**：提示詞層已落地、抽驗 2/3 案例可回溯到主席，但 `二、科長指示及提醒事項` 仍混入非主席細節，且**沒有**結構化的「裁示→發言者＋時間＋quote」欄位（S3 未實作）。是否算通過 C2，須由 Stage 05 依 plan.md 判定。
...
4. **ASR 文字品質本身**：逐字稿有大量同音錯字（例如「編織錶」「雞茶股」）——這是 ASR 層，不是 diarization/LLM 層；語意校正本次因 LM Studio 未開而跳過。
