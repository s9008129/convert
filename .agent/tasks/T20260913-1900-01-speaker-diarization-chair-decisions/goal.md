...
- **ASR 語意／錯字校正不在本 GOAL 範圍**：那是使用者另一個專案的獨立任務；本任務只做「發言者分離＋主席裁示歸屬」。若使用者要求移植，先開新任務與新計畫。**未來方向（使用者 2026-09-13 補充修正，取代先前「一律用雲端」之指示）**：校正引擎**跟隨使用者選擇的處理模式**——選**地端模式**就用**地端模型**校正，選**雲端模式**就用**雲端模型**校正；本任務（收尾／驗收）不執行校正，移植實作時依此方向落地。
