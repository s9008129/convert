...
| 場次 | 模型 | 期望／遺漏（topic） | 期望／遺漏（decision） | 期望／遺漏（number） | 期望／遺漏（date） | 紀錄字元 | `coverage_all`／`core` | 任務耗時 | verdict |
| --- | --- | --- | --- | --- | --- | ---: | --- | ---: | --- |
| **E6b** | qwen3.8-27b-splash | 14／0 | **27／0** | 9／0 | 4／0 | 4,853 | 0.8657／0.9643 | **1,023.2 s** | **PASS（16/16）** |
| **E6** | gemma-4-31b-it-MLX-4bit | 9／0 | **11／0** | 9／**1** | 4／0 | 2,184 | 0.6269／0.75 | **2,086.3 s** | **PASS（16/16）** |

- 修補前後同模型同素材：1,662.1 s → **1,023.2 s**（未膨脹，反降）；`coverage_all` 0.8507 → **0.8657**。
- **換模型同場景（模型無關性）**：gemma 場決議期望集合 11、遺漏 0；同模型同素材 P5 對照
  0.5522／0.7143 → 本場 **0.6269／0.75**。同一支 runner、同一模板、同一支音檔、只換模型
  ⇒ 解析層修補對兩顆模型都生效（不是為 qwen 特調的模型名分支）。
- **新事實（如實登錄、不外推）**：gemma 紀錄明顯比 qwen 短（2,184 vs 4,853 字）、核心覆蓋
  0.75 vs 0.9643，且 2 輪補強後仍有 1 個數字（`600`）未收斂；方向與「gemma 體感較精準」的
  主觀印象不同，已寫進 gemma 場 README 與手冊 §8-17，不代為解釋成因。
...
4. **補強收斂上限**：gemma 場 2 輪補強後仍剩 1 個數字（`600`）未補進紀錄 → 輪數上限（預設 2）
   造成的殘餘遺漏，非 no-op；調高輪數是成本／品質取捨，本波未變更。
