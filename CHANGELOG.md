...
### 🎯 主題：P7-A——地端補強輪「不回退」守衛（交付版本不得比本次執行中曾達到的最佳版更差）

**意圖**：補強輪是**整份紀錄重生成**（不是局部 patch），所以「最後一版」不保證是最好的一版；
使用者拿到的卻是最後一版。本波唯一目標：**交付版本的核心覆蓋率不得低於本次執行中曾達到的最佳值**。
這是確定性守衛（用既有的逐條對帳統計），不是提示詞調參、也不是模型名分支。

**根因（不是模型不夠力，是流程語意）**：同一支音檔（0903-科務會議）、同一模板（`section_meeting`）、
同一顆模型（`gemma-4-31B-it-MLX-4bit`）、同一任務的三次逐條對帳顯示數字類未涵蓋數
`3 → 0 → 又 1`（log 時間點 15:28／15:35／15:42，證據 `data/cache/e2e/p6a-gemma31b-e6/backend.log`）——
第 1 輪補強把 3 個數字補回來，第 2 輪重生成又把其中的 `600` 寫掉。補強迴圈只保留「當輪輸出」，
沒有「本次執行最佳版本」的任何記憶，也沒有跨輪比較；逐條對帳（P4-A）自始只把結果做成問題清單與
metrics，**從來沒有拿來做跨輪比較**。

### 🔧 修正（`3e9311a`；只在地端路徑）

1. `backend/core/config.py`：新增 `LOCAL_LLM_REFINEMENT_NO_REGRESSION`（bool，**預設 `True`**）。
   **關閉方式＝一行設定**：`LOCAL_LLM_REFINEMENT_NO_REGRESSION=false` ⇒ 回本波前
   （交付最後一版、**無任何守衛 log 行**）。
2. `backend/services/summarization.py`：
   * `_validate_local_record()`：把「初稿後／每輪補強後」原本各自手寫的同一組確定性檢查
     （品質／發言者標註／年份錨定／逐條對帳／忠實度）抽成單一入口，順序與項目一字不變。
     理由不是重構美感：回退路徑必須用**同一組檢查重算**，否則 log／`cov_*` 會與實際交付版本不一致。
   * `_core_coverage_snapshot()`：回傳核心覆蓋快照 `(未涵蓋數, 期望數, 未涵蓋項目身分)`；
     逐條對帳未執行（`off`）或期望集合為 0 ⇒ 取不到快照（守衛不作用、不誤傷）。
   * `_refinement_regression_reason()`（純函式）：回退判定兩條——①未涵蓋數變多；
     ②未涵蓋數相同但**換項**（等量互換：原本寫到的事實被寫掉、別的補回來）。
     刻意**不採**「掉了就回退」的嚴格版——那會為保住 1 項而放棄同輪淨修好的多項。
   * 補強迴圈：逐輪維護 `best_summary`／`best_snapshot`；判定回退 ⇒ **丟棄該輪輸出**、
     改交付最佳版本（同分取最後一版），**並以同一組檢查重算問題清單與 `cov_*`**；
     持平／更好 ⇒ 更新最佳版本。期望集合中途變動（基準不同不可比）或快照消失 ⇒
     本場停用比較、交付最後一版，留下說明 log。
   * `_validate_record_source_coverage()`：新增 additive 內部鍵 `missing_items_<類別>`
     （未涵蓋**項目身分**，供集合比較）；**不進** `cov_*` metrics 行、不改既有欄位格式。
3. 比較基準刻意**只有核心覆蓋率**（議題／決議／數字／日期），**不含忠實度與長度**——
   避免出現「用捏造換事實」的反向交易。

### 📜 契約修訂（重要）

`LOCAL_LLM_RECORD_COVERAGE_MODE=observe` 下，逐條對帳的**問題清單**仍零變化，
但**交付版本**可被本守衛換成較佳版本——「observe＝品質零變化」這句話的適用範圍自此**限縮為
「問題清單」**。九個 `cov_*` 欄位名稱與格式不變，且回退後一律重算，數值對應實際交付版本。

### 📏 觀測字串（釘為契約）

| 情境 | log 字串 | 級別 |
|---|---|---|
| 每輪比較（守衛有作用） | `地端補強不回退守衛：第 N 輪核心未涵蓋 X → Y 項（期望 Z 項）（持平，取本輪）`／`（更好，取本輪）` | info |
| 觸發回退 | `地端補強第 N 輪造成事實回退（<原因>）：核心未涵蓋 X → Y 項（期望 Z 項）→ 丟棄本輪輸出，交付最佳版本（N 字元）`；`<原因>`＝`未涵蓋數變多` 或 `等量互換（原本已寫到的事實被寫掉 K 項）` | warning |
| 停用比較（本場不再受守衛影響） | `地端補強不回退守衛：第 N 輪取不到核心覆蓋快照（逐條對帳未執行）→ 本場停用比較，交付最後一版`／`…期望集合由 X 項變為 Y 項（基準不同不可比）→ 本場停用比較，交付最後一版` | info |

### 🧪 驗證

- 全套單元＋整合測試：`DATA_DIR=/tmp/probe_scratch_p7a uv run --frozen python -m pytest tests/ -q
  --ignore=tests/test_end_to_end.py` ⇒ **1100 passed, 2 skipped**（P6-A 基線 1088 → +12；
  新檔 `tests/test_t20260923_p7a_refine_no_regression.py` T01–T12，含「回退後問題清單與 `cov_*`
  對齊交付版本」「`observe` 供快照、`off` 完全不作用」「關閉開關逐字等同本波前路徑」）。
- 離線重播（真實素材、呼叫產品同一份判定函式）：
  `.agent/tasks/T20260923-1700-03-local-refine-no-regress/e2e/attempt-P7A-offline-replay/`
  （`replay_guard.py`／`output.json`）——**核心未涵蓋**較佳版 **26／46**、真實交付版 **27／46** ⇒
  `guard_would_revert: true`、原因「未涵蓋數變多」。即：**這次真實失敗場，本守衛會把交付版本
  換回較佳的那一版**。
- 正式 E2E（`--quality-mode observe`、`section_meeting`、同一音檔）：場次
  `e2e/attempt-P7A-gemma31b-e7c` **已排定／進行中，結果尚未產生**；在 append-only 登錄完成前，
  不得宣稱任何 E2E 結論（含「守衛已生效」）。

### ⚠️ 未達項與殘餘風險（如實登錄）

1. **不提升初始生成品質**，也不推論「地端已達雲端 Gemini 水準」；本波只是讓交付版本
   「不低於本次執行曾達到的最佳版」。
2. **不比忠實度**（已知邊界）：守衛只認核心覆蓋率；若某輪靠捏造換取覆蓋率提升，守衛會採用該輪
   （P4-B 忠實度絆索是唯一緩解）。等量互換判定只認「新掉的項」——掉了多項但淨改善
   （例如 3 項→1 項、其中 1 項是新掉的）**不判回退**，這是刻意的取捨（保住淨改善）。
3. **字面比對的繼承限制**：逐條對帳是字面量尺——同一事實換寫法可能被判成「漏寫」
   （例：數字寫成中文數字、議題句換句話說）而觸發回退，放棄一個其實沒有比較差的候選版本；
   回退的代價上限＝維持前一版（不會弄壞內容）。這是既有對帳器（P4-A）的精度上限，本波不處理。
4. **等量互換判定的已證實邊界** `[VERIFIED]`：第②條是用逐條對帳的**未涵蓋項目身分**比較，
   所以某輪把同一件事**換句話說**（例：原本寫「600 元」的項目漏掉、新版寫成「六百元」補上該項
   卻漏掉另一項）也可能被判成「互換」而回退——**最壞後果只是維持前一版不換版**（保留較佳文字、
   不會弄壞內容、不會刪句），代價是可能放棄其實有進步的那一輪。這是刻意取捨：嚴格版「掉任何一項
   即回退」更糟，會為保住 1 項而放棄同輪淨修好的多項。
5. **回退後下一輪即停止**：回退會讓問題集合回到驅動該輪的集合，下一輪頂端命中既有不收斂保護
   而停止（**不多花輪次**）；代價是回退後不再嘗試其他修法。
6. **`Windows 11 ＋ Ollama` 實機未驗** `[UNVERIFIED]`：機制為純 Python 條件式＋既有 `cov_*` 統計，
   無平台／引擎分支，但 4090 實機 A/B 未跑（本機為 macOS）。
7. 單場 E2E 為單次抽樣（temperature ≠ 0）：跨場比較含抽樣雜訊，「變快／變好」的宣稱
   需同 build 至少 2 場。

### 📌 下一步建議（P7-A 之後）

1. 完成正式 E2E 驗收登錄（append-only，`e2e/attempt-P7A-gemma31b-*`）：確認 `backend.log`
   出現守衛 log、交付版本為最佳版本，並如實登錄結果（不預寫結論）。
2. 後續優先序（依根因排序；①本波已實作、待 E2E 登錄）：**①補強輪整份重生成回退 → ②萃取筆記
   資訊量（gemma 筆記僅 qwen 的 39%）→ ③逐條化過度＋`（待確認）`殘留**（②③見研究文件 §12.11）。
3. `Windows 11 ＋ RTX 4090 ＋ Ollama` 實機驗收：用同一支音檔、同一模板（`section_meeting`）
   跑一場，才能把「跨 OS／跨引擎」由靜態稽核升級為實機證據 `[UNVERIFIED]`。

## [v4.10.2] - 2026-09-23

### 🎯 主題：P6-A——修掉決議類逐條對帳的「整類靜默 no-op」（真實 0903 場實證）

「決議期望集合為空」過去只留一行 warning：不報錯、不補強、不影響 verdict。實測它不是罕見情境——
`qwen3.8-27b-splash` ＋ `section_meeting` 模板在真實 0903 素材上，E4／E5／E5b **連續三場**
`cov_expected_decision=0`，同一批場次的議題期望集合卻有 9／12／15 → 決議漏寫等於**沒有保障**。
根因是 P4-A 家族的「單一寫法」解析器：筆記換一種排版就靜默退化，且退化本身沒有症狀。

### 🔧 修正（P6-A `7139d44`＋邊界硬化 `4b707b6`；全部在 `backend/services/summarization.py`，純解析層）

1. **空殼標記＋巢狀內容**：決議寫成 `- *決議*：`（標記行本身沒內容），內容在下一層條列 →
   舊解析器只看標記行 ⇒ 決議期望集合恆為 0。新增 `_collect_nested_notes_items` 收集
   「縮排更深的巢狀條列」，遇同層縮排／`#` 標題即停。空殼判定＝標記行**剝掉引用標頭後沒有內容**，
   同時涵蓋 `- *決議*：`、`- *決議*：[00:06:14] 發言者 1（主席裁示）：` 與整行粗體 `- **決議**`。
2. **時間＋發言者同框的引用標頭**：`[00:12:05 發言者1（主席）]：…` 舊樣式剝不掉，整段標頭
   （約 20 字）留在正文、LCS 比對被吃掉一半（同 E1 成因）。新增
   `_NOTES_ITEM_BRACKET_CITATION_PATTERN`＝既有「只有時間」樣式的**超集**（舊樣式結果不變）。
3. **編號型條列**：模型常自行寫 `1.`／`1)`／`（1）`；舊樣式只認 `-`／`*`／`+`，整份筆記換一種
   編號就全數抽不到。新增 `_NOTES_ITEM_BULLET_PATTERN`。
4. **標記同義詞與共用過濾**：決議標記詞加入 `決議事項`／`決議內容`；巢狀路徑與直接路徑共用同一套
   過濾（長度下限 6 字／佔位詞／同鍵去重）。**刻意不放寬**：`*討論重點*`／`裁示`／`結論` 仍不算決議；
   議題類遇到 `- **決議**` 不再把「決議」兩字當成一條議題。
5. **巢狀邊界硬化（獨立稽核後的對抗性修正）**：角色標頭判定改「冒號**或空白**或行尾」，
   巢狀區塊內省略冒號的角色標籤（`- *討論重點*`）不再被收成本類內容（否則會產生假的
   「決議遺漏」補強問題、把討論重點污染成決議）；反之，**同角色**的巢狀重述
   （`- 決議：通過`）不再被當成標頭丟掉。`- 決議文康活動形式為…` 這種「角色詞當內容開頭」
   的列不會被誤切。此為 `4b707b6`，來自我請獨立稽核（fresh context、對抗性輸入）複核的發現。
6. **雲端路徑 byte 級不變**：呼叫點只有地端（首輪與每輪補強重算）。

### 📊 實測

**修補前（問題證據）**：0903 場 qwen27B 連續三場決議期望集合為空 →

| 場次 | `cov_expected_topic` | `cov_expected_decision` | 證據位置 |
| --- | ---: | ---: | --- |
| E4 | 9 | **0** | `data/cache/e2e/p4-qwen27b-e4/backend.log:284` |
| E5 | 12 | **0** | `data/cache/e2e/p4-qwen27b-e5/backend.log:125` |
| E5b | 15 | **0** | `data/cache/e2e/p4-qwen27b-e5b/backend.log:427` |
| （對照）gemma E5b | 9 | 10 | `data/cache/e2e/p4-gemma31b-e5b/backend.log:523` |

**修補後（真實 E2E，同一支 `0903-科務會議.m4a`、同一模板 `section_meeting`、`--quality-mode observe`）**：

| 場次 | 模型 | 期望／遺漏（topic） | 期望／遺漏（decision） | 期望／遺漏（number） | 期望／遺漏（date） | 紀錄字元 | `coverage_all`／`core` | 耗時（runner wall／task） | verdict |
| --- | --- | --- | --- | --- | --- | ---: | --- | ---: | --- |
| **E6b** | qwen3.8-27b-splash | 14／0 | **27／0** | 9／0 | 4／0 | 4,853 | 0.8657／0.9643 | **1,023.2／1,015.4 s** | **PASS（16/16）** |
| **E6** | gemma-4-31b-it-MLX-4bit | 9／0 | **11／0** | 9／**1** | 4／0 | 2,184 | 0.6269／0.75 | **2,086.3／2,076.4 s** | **PASS（16/16）** |

- 修補前後同模型同素材（runner wall／task）：1,670.5／1,662.1 s → **1,023.2／1,015.4 s**（未膨脹，反降）；
  `coverage_all` 0.8507 → **0.8657**。（耗時基準見下方更正說明：一律標「runner wall／task」兩個數字。）
- **換模型同場景（模型無關性）——獨立驗收後更正**：gemma 場決議期望集合 11、遺漏 0；但同模型
  同素材的 P5 對照場（`e2e/attempt-E5-gemma31b-p5b/`，跑在修補前 revision `ea510e5`）決議本來就是
  **10／0**（`data/cache/e2e/p4-gemma31b-e5b/backend.log:523`）⇒ 本場只能說「修補**沒有回歸**、
  決議抽取不依賴模型名、沒有為 qwen 特調的模型名分支」，**不能**說「修補對 gemma 生效」。
  `coverage_all`／`core` 0.5522／0.7143 → 0.6269／0.75 是單場差，**不可單獨歸因**本修補
  （同 build 單場擺動可達 16 pp，見手冊 §8-16）。
- **新事實（如實登錄、不外推）**：gemma 紀錄明顯比 qwen 短（2,184 vs 4,853 字）、核心覆蓋
  0.75 vs 0.9643，且 2 輪補強後仍有 1 個數字（`600`）未收斂；方向與「gemma 體感較精準」的
  主觀印象不同，已寫進 gemma 場 README 與手冊 §8-17，不代為解釋成因。其中 `600` 這個數字的
  確切成因經獨立驗收更正為「補強輪回退」而非輪數不足，見下方第 4 條與手冊 §8-18。
- 過程中確實逮到漏項並自動補強（qwen 場）：14:57:01 log「紀錄覆蓋率比對（決議）：未涵蓋 3 項…」＋
  「（數字）：未涵蓋 4 項：600、800、17、13600」→ 第 1 輪補強後最終 `cov_missing_*=0`、
  `cov_issues_added=0`（收斂）。gemma 場同樣在 15:28／15:35 兩輪長出補強問題（決議／數字／日期／
  待辦／表格欄位），最終決議 missing 0。
...
- 新增／更新測試：T-20（預期值隨語意變更同步）、T-30～T-37（真實筆記形狀、整行粗體標頭、
  標記同義詞、編號條列、巢狀邊界、同框引用標頭、無冒號角色標籤、角色詞當內容開頭）。
- 獨立稽核 red 對照（baseline `29c39a5` 另開 worktree）：T-20 與 T-30～T-35 皆紅、其餘 26 項綠；
  修補版同檔全綠 → 新測試非空轉。效能：真實 4,865 字筆記 decision 解析 p50 0.29 ms、
  病態單行 16k 字 <0.15 ms，無災難性回溯。
- 真實 E2E：`e2e/attempt-P6A-qwen27b-e6b/`（qwen）與 `e2e/attempt-P6A-gemma31b-e6/`（gemma）
  兩場皆 runner verdict PASS、16/16 checks、`failure_reasons=[]`，DOCX 正常產出。

### ⚠️ 未達項與殘餘風險（如實登錄）

1. **B2-a 已知限制**：模型若把空殼標記的內容寫成**同層（同縮排）**條列，本機制仍會靜默 no-op。
   真實兩顆模型（qwen／gemma）的筆記都是更深一層，故本波不做「同層收斂」（會引入誤收風險）。
2. **同家族未修**：`（疑似：…）` 紀錄層標註器不存在、待辦表格只認 `|`≥3 欄、`(待確認)` 半形漏網、
   數字單位白名單偏窄、日期漏 `M-D`／中文數字、heading scoping 全有全無。
3. **`Windows 11 ＋ Ollama` 實機未驗** `[UNVERIFIED]`：本修補為純解析層、無 OS／模型名分支
   （跨 OS／跨引擎靜態稽核見 `e2e/crossos-p5-audit-01/report.md`），但 4090 實機 A/B 未跑。
4. **補強輪是「整份重生成」，會把已達標事實寫掉（獨立驗收更正；不是輪數不足）**：gemma 場三次
   對帳時點（`data/cache/e2e/p6a-gemma31b-e6/backend.log:328／421／511`）顯示——15:28（首輪）
   數字缺 3 項（`15／600／100`）、15:35（第 2 輪補強前）數字缺 **0** 項、15:42（第 2 輪補強後）
   又缺 `600` 1 項；決議類同場也是「15:35 補回一項、同時掉了另一項（`10/14 排程`）」。
   ⇒ 殘餘遺漏來自**重生成回退**（打地鼠），不是「預設 2 輪不夠」；調高輪數只是把同一個回退風險
   多跑幾次。對症解＝重生成差異保護／局部編輯（P7 候選，見下一步建議第 2 條）。判讀法見手冊 §8-18。
5. **語意變更已登錄**：本波放寬的是「什麼算一條決議」的**形狀容忍度**（對帳因此更嚴），
   未動任何門檻、未動 `off` 模式、未改雲端提示詞。回滾＝`git revert 7139d44 4b707b6`。
6. **本節數據更正（獨立驗收後補登，不改既有證據目錄）**：①耗時一律標「runner wall／task」兩個數字
   （混用會讓「變快／變慢」的結論失真）；②`600` 的成因由「輪數上限」更正為「重生成回退」（第 4 條）；
   ③「修補對 gemma 生效」更正為「未回歸、非模型名分支」（見上方換模型段落）。原始產物未動，
   更正以新增條目與 README 補登方式呈現。

### 📌 下一步建議（P6-A 之後）

1. ~~決議類逐條對帳 no-op~~ 已完成（`7139d44`／`4b707b6`，qwen／gemma 兩顆模型各一場 PASS
   16/16）；接續 **B2-a**：模型若把空殼標記的內容寫成**同層（同縮排）**條列，本機制仍會 no-op
   （要放寬得先有誤收防護測試）。
2. **P7 候選：補強輪回退保護**（本波驗收後新發現，見未達項第 4 條與手冊 §8-18）：每輪補強是
   整份重生成，會把已達標事實寫掉。選項：①補強提示夾帶「已達標、不得刪除」事實清單；
   ②改成局部編輯（只動有問題的段落）；③重生成後比對「已達標集合不得縮小」，縮小就回退上一版。
   任一選項都需要同素材 2 場 A/B＋一行回退開關，且不得動既有門檻。
3. 未修家族項逐項評估：`（疑似：…）` 紀錄層標註器、待辦表格欄位、`(待確認)` 半形、
   數字單位白名單、日期 `M-D`／中文數字、heading scoping 全有全無。
4. 在 **Windows 11 ＋ RTX 4090 ＋ Ollama** 用同一支音檔、同一模板（`section_meeting`）跑一次實機驗收，
   才能把「跨 OS／跨引擎」由靜態稽核升級為實機證據 `[UNVERIFIED]`。
