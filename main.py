...
範例:
    ...
    if system_prompt:
        ...
        if "資深行政秘書與公務紀錄專家" in system_prompt and "主辦單位" in system_prompt:
            logger.info("[品質] 已啟用正式公務紀錄約束機制 ✓")
        else:
            logger.warning("[警告] 系統提示詞未包含正式公務紀錄格式約束")
            logger.warning("[建議] 更新 config.yaml 的 system_prompt 以啟用新格式檢查")
    else:
