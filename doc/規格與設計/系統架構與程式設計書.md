...
| 文件版本 | v4.3.3 |
| 文件日期 | 2026-07-14 |
| 系統版本 | 4.3.3（唯一來源：根目錄 `VERSION` 檔，經 `backend/core/version.py:15-26` 讀取） |
| 適用讀者 | 開發者、維護工程師 |
| 專案根目錄 | `D:\dev\convert` |

> 本文件所有技術細節均附 `檔案:行號` 引用，行號以 2026-07-14 的 `main` 分支（commit `e0054e3`）為準。
> 已知不一致：`pyproject.toml:15` 的 `version = "4.2.0"` 未同步更新；系統實際版本一律以 `VERSION` 檔（4.3.3）為準。
...
| 雲端 LLM | Gemini API（OpenAI 相容端點，預設 `gemini-3.1-flash-lite`） | `config.py:76-80` |
...
| `GEMINI_MODEL` | `gemini-3.1-flash-lite` | 雲端模型 | `config.py:80` |
