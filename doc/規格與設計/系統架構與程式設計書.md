...
| 文件版本 | v4.7.1 |
| 文件日期 | 2026-08-20 |
| 系統版本 | 4.7.1（唯一來源：根目錄 `VERSION` 檔，經 `backend/core/version.py:15-26` 讀取） |
| 適用讀者 | 開發者、維護工程師 |
| 專案根目錄 | `D:\dev\convert` |

> 本文件所有技術細節均附 `檔案:行號` 引用。第 6.4、8.5、9、10、11.2、11.4、13、14、15、16 節（v4.6.2–v4.7.1 異動部分）已於 2026-08-20 對照 `main` 分支（commit `2ca9595`）逐一核對；其餘章節沿用各自新增/最後核對當下的版本，未隨後續版本重新核對（`git diff` 已確認 `transcription.py`／`correction.py`／`text_postprocess.py`／`glossary.py`／`device_detector.py`／`file_manager.py`／`queue_manager.py`／`websocket.py`／`main.py` 等自 v4.5.0 起未變動，故其行號應仍有效）。
> v4.4.0 會議模板系統、v4.5.0 科務會議模板與附件、v4.6.0 ISMS 月工作會議模板與表單式 DOCX、v4.6.1 DOCX 章節白名單修復均見第 6.5 節（該節內容已是最新、行號以各自新增當下版本為準）。
> v4.6.2–v4.7.1「地端管線韌性機制」（空白會議紀錄根因與根治）見第 6.4、11.4 節。
...
│   │   ├── platform_config.py  # 平台偵測；config.yaml 僅供舊版 CLI／legacy tooling 讀取
│   │   ├── logger.py           # loguru 日誌
...
| `platform_config.py` | 平台偵測（macos/windows/docker）；讀取 `config.yaml`＋`config.{platform}.yaml`——**僅**供舊版 CLI 使用（LM Studio 參數 YAML 覆蓋路徑已於 T20260827-1127-01 移除） | `platform_config.py:68-110` |
...
**Ollama 呼叫細節**（`_summarize_with_ollama`，`summarization.py:1118-1246`；請求/串流/重試層見第 6.4 節，不在此重複）：

- `keep_alive` 採設定值（預設 `30m`，v4.7.0 由 `10m` 調高，`config.py:107-110`）：三階段流程連續呼叫十數次，`keep_alive=0` 會導致每階段重載 20GB 模型（P0-7 註解：`summarization.py:1175-1177`）。v4.7.0/v4.7.1 起呼叫鏈全面串流化＋可重試分類＋`done_reason=length` 接受路徑＋`num_predict` 自動擴大，詳見第 6.4 節。
- gemma4 是思考型模型，thinking 會吃光 `num_predict` 導致正文為空 → 預設帶 `think: false`；模型不支援該參數回 400 時，自動改以不帶 `think` 欄位重送（`summarization.py:1064-1074`）。
- 空回應自動重試一次並升溫（`temperature → max(t, 0.3)`，`summarization.py:1166-1206`）。
- 404（模型不存在）時列出主機可用模型與 `ollama pull` 建議指令（`summarization.py:1218-1240`）。
- 模型名智能解析 `_resolve_compatible_model`：4 個策略（正規化量化後綴同名 → 同家族前綴 → 同家族 `:latest` → 任一同家族），設定 `gemma4:31b` 而主機裝 `gemma4:31b-it-q4_K_M` 時自動對上（`summarization.py:1680-1742`）；生效名經 `get_effective_local_model()` 提供給 `/api/config` 供前端顯示（`summarization.py:1773-1775`、`routes.py:446`）。
...
```mermaid
flowchart TD
    A["ASR 前 release_local_model()<br/>task_processor.py:111-113"] --> B["warmup_local_model()<br/>load-only + /api/ps 偵測 offload<br/>summarization.py:1925-2008（詳見 11.4 節）"]
    B --> C{"offload？"}
    C -->|"否"| D["_summarize_with_ollama()<br/>num_ctx=預設 (1118-1246)"]
    C -->|"是，自我修復一次"| E["卸載→等待→重載<br/>(1969-2001)"]
    E -->|"仍 offload"| G["本任務降級 num_ctx=8192<br/>(1983-1999)"]
    E -->|"修復成功"| D
    G --> D
    D --> H["_stream_ollama_chat_once()<br/>閒置逾時120s／總時長1800s<br/>(964-1024)"]
    H --> I{"done_reason？"}
    I -->|"stop"| J["正常回傳"]
    I -->|"length（v4.7.1 起）"| K["接受內容＋WARNING<br/>不重試 (1078-1085)"]
    I -->|"其他異常/連線中斷"| L["OllamaStreamRetryable<br/>_post_ollama_chat 重試 ×2<br/>(1050-1108)"]
```

| 機制 | 說明 | 位置 |
| :--- | :--- | :--- |
| 例外訊息保底 | `describe_exception(e)`：`str(e)` 為空時（httpx 逾時類例外的常見行為）回退帶出例外類別名稱，避免「失敗原因」整行消失 | `backend/core/errors.py:10-13` |
| Ollama 呼叫全面串流化 | `stream:false` 改 `stream:true`；httpx read timeout 降級為 chunk 間「閒置」逾時（`LOCAL_LLM_STREAM_IDLE_TIMEOUT=120s`），另以 `asyncio.timeout` 作總時長上限（`LOCAL_LLM_REQUEST_TIMEOUT=1800s`，寧可等待不截斷） | `_stream_ollama_chat_once`：`summarization.py:964-1024` |
| 串流可重試例外分類 | `OllamaStreamRetryable`：中途 error chunk／結束無 done／`done_reason` 異常（非 stop、非 length）才可重試 | 類別定義：`summarization.py:32-43`；判定：`summarization.py:1000-1022` |
| `done_reason=length` 接受路徑（v4.7.1） | 撞到 `num_predict` 上限是**確定性結果**，重試 2 次必然停在同一截斷點；不再拋可重試例外，直接回傳已完整累積內容，呼叫端記 WARNING（含 `eval_count`/`num_predict`）後接受繼續 | 串流層判斷：`summarization.py:1016-1022`；呼叫端警告：`summarization.py:1078-1085` |
| 瞬時錯誤重試 | `_post_ollama_chat`：timeout/連線中斷/串流層可重試失敗才重試，`LOCAL_LLM_TRANSIENT_RETRIES=2` 次、線性退避 `LOCAL_LLM_RETRY_BACKOFF_SECONDS=5.0`；HTTP 4xx/5xx 與總時長超限非瞬時，不重試 | `summarization.py:1050-1108` |
| 生成指標觀測 | 每次生成記錄 `load_duration`／`tokens/s`／`prompt_eval_count`（v4.7.1 新增）；吞吐低於 `LOCAL_LLM_MIN_TOKENS_PER_SECOND=5.0` 即警告疑似 offload | `_log_generation_metrics`：`summarization.py:1027-1048` |
| `num_predict` 自動擴大（v4.7.1） | 依 context 餘裕自動擴大輸出預算（`max(保留值, num_ctx − 估算prompt − 256)`），從源頭降低撞牆機率；整併呼叫 `_merge_notes_until_fit` 明確傳 `expand_output_budget=False` 維持「整併必縮小」的收斂保證 | `_summarize_with_ollama`：`summarization.py:1143-1151`；整併停用理由註解：`summarization.py:763-766` |
| 任務級 num_ctx 生效順序 | 顯式覆蓋 > warmup 降級值 > settings 預設 | `_effective_context_tokens`：`summarization.py:1110-1116` |
| Warmup 自我修復＋降級 | load-only 預熱→`/api/ps` 偵測 offload→自我修復一次（卸載/等待/重載）→仍失敗則本任務降級 `num_ctx=LOCAL_LLM_DEGRADED_CONTEXT_TOKENS`（8192） | `warmup_local_model`：`summarization.py:1925-2008`（詳見第 11.4 節） |
| KV 量化生效 heuristic | `/api/tags` 磁碟大小 vs `/api/ps` 載入大小之差 ≈ KV cache＋compute overhead；依 2026-08-19 正式機實測校準門檻（q8 生效 @16K overhead≈2.7GB） | `_check_kv_quantization_heuristic`：`summarization.py:1903-1923` |
| ASR 子程序隔離 | 每次轉錄改在獨立子程序執行，程序退出保證 CUDA context／分配器殘留完全釋回（faster-whisper#992 實測長駐程序每次殘留 ~312MB） | `backend/services/asr_subprocess.py:78-146`、`backend/workers/asr_worker.py`（詳見第 11.2 節） |
...
摘要拋錯時：保留逐字稿輸出、標題改「# 逐字稿（會議紀錄生成失敗）」、冠 `SUMMARY_FAILED_BANNER` 警告＋失敗原因，任務仍標 `COMPLETED` 讓使用者至少拿到逐字稿（banner 定義：`task_processor.py:32-35`；例外捕捉＋`task.summary_failed=True`：`task_processor.py:229-235`；標題／警告組裝：`task_processor.py:316-322`）。「失敗原因」行改為無條件輸出（`reason = summary_error or "未知錯誤（無例外訊息）"`，v4.6.2 修復——空例外訊息曾讓此行整個消失，見 `describe_exception`）。v4.6.2 起下載檔名也依 `task.summary_failed` 動態變化（`"逐字稿(會議紀錄生成失敗)"` vs `"會議紀錄"`），與標題/警告同步一致，避免使用者把 fallback 文件誤認為正式紀錄（欄位定義：`schemas.py:52`；套用：`routes.py:290`、`333`、`364`；詳見第 9.1 節第 5 項）。
...
> 註：本表行號已於 2026-08-20 逐一核對修正——舊表格行號自 v4.4.0/v4.5.0 模板系統擴充後即未同步，drift 已達 20 行以上（例如端點 5 舊標 `213-293`，實為 `230-365`）。

| # | 方法/路徑 | 功能 | 重點行為 | 位置 |
| :--- | :--- | :--- | :--- | :--- |
| 1 | `GET /health?quick=` | 健康檢查 | `quick=true` 用快取裝置資訊（防 GPU 滿載阻塞）；回版本、GPU、Ollama/LM Studio/Gemini 可用性、佇列狀態 | `routes.py:33-90` |
| 2 | `POST /upload` | 上傳檔案 | 驗格式（`file_manager.py:51-76`）→ 驗大小（413）→ 驗模式（400）→ 雲端模式檢查 API Key（400）→ 存檔（uuid4 前 12 碼檔名，`file_manager.py:108-110`）→ 入佇列；佇列滿刪檔回 503 | `routes.py:93-184` |
| 3 | `GET /tasks/{id}` | 查任務進度 | 不存在回 404；即時更新排隊位置 | `routes.py:187-203` |
| 4 | `GET /tasks/{id}/transcript` | 下載逐字稿 | 純文字 `.txt`；轉錄＋校正完成即產生，摘要失敗仍可下載；下載檔名 `YYYYMMDDhhmmss_逐字稿.txt` | `routes.py:206-227` |
| 5 | `GET /tasks/{id}/result?format=md\|docx` | 下載會議紀錄 | 僅 `COMPLETED` 可下；`docx` 動態轉檔＋mtime 快取（`.docx` 比 `.md` 舊才重轉）；下載檔名依 `task.summary_failed` 動態切換：正常為 `YYYYMMDDhhmmss_會議紀錄.{md,docx}`，摘要失敗 fallback 為 `YYYYMMDDhhmmss_逐字稿(會議紀錄生成失敗).{md,docx}`（v4.6.2） | `routes.py:230-365`（`record_label` 判斷：`routes.py:290`） |
| 6 | `DELETE /tasks/{id}` | 取消任務 | 僅排隊中可取消；處理中回 400 | `routes.py:368-392` |
| 7 | `GET /queue/status` | 佇列整體狀態 | 回 `QueueStatus` | `routes.py:395-400` |
| 8 | `GET /queue/position/{id}` | 查排隊位置 | 位置＋總排隊數＋預估等待 | `routes.py:403-421` |
| 9 | `GET /config` | 公開設定 | `max_file_size_mb`、`allowed_extensions`、`gemini_available`、`local_llm_model`（**解析後生效名**）、`cloud_llm_model`、ASR 模型與 revision 等——前端模式卡顯示唯一來源 | `routes.py:424-451` |
| 10 | `GET /storage/stats` | 儲存統計 | 各目錄檔數/大小＋保留政策 | `routes.py:454-465` |
| 11 | `POST /storage/cleanup` | 手動清理 | 僅清過期檔案 | `routes.py:468-477` |
| 12 | `GET /gemini/health?force_refresh=` | Gemini 連線檢查 | 24 小時快取（快取初始化：`summarization.py:127-131`；`check_gemini_health`/`reset_gemini_health_cache`：`summarization.py:1795-1857`）；`force_refresh=true` 強制重測 | `routes.py:480-512` |
...
| `summary_failed` | bool | 完成但僅含逐字稿（會議紀錄生成失敗），預設 `False`（v4.6.2 新增，`schemas.py:81`；`TaskInfo` 同名欄位：`schemas.py:52`；`task_processor._update_progress` 帶入：`task_processor.py:67`） |
...
由 pydantic-settings 實作（`config.py:363-366`：`env_file=".env"`、`extra="ignore"`）。
...
> 註：`TASK_TIMEOUT_SECONDS` 已於 v4.6.2 移除——整任務 `wait_for` 對長會議是合法超時，且無法安全取消 executor 中的 ASR；超時控制改在每次 LLM／ASR 請求層級（`LOCAL_LLM_REQUEST_TIMEOUT`、`ASR_WORKER_TIMEOUT_SECONDS` 等，見下表），移除理由註解於 `config.py:38-39`。下表其餘既有鍵的行號因 v4.6.2/v4.7.0 於檔案中段插入約 50 行新設定而整體下移，已於 2026-08-20 逐一重新核對。

| 設定鍵 | 預設值 | 說明 | 位置 |
| :--- | :--- | :--- | :--- |
| `DEFAULT_MODE` | `local` | 預設處理模式 | `config.py:90` |
...
| `ESTIMATED_MINUTES_PER_TASK` | 4 | 排隊等待估算單位 | `config.py:40` |
| `OLLAMA_BASE_URL` | `http://host.docker.internal:11434` | Ollama 端點 | `config.py:45-48` |
| `LOCAL_LLM_MODEL` | `gemma4:31b` | 本地模型（可被智能解析替換為量化變體） | `config.py:49-52` |
| `LMSTUDIO_BASE_URL` / `LMSTUDIO_MODEL` | `http://host.docker.internal:1234/v1` / `gpt-oss-20b` | LM Studio 備援 | `config.py:55-59` |
| `GEMINI_API_KEY` | None | 雲端金鑰（格式驗證：≥10 字元、僅英數 `_-`） | `config.py:61-76` |
| `GEMINI_MODEL` | `gemini-3.5-flash-lite` | 雲端模型 | `config.py:81` |
| `CLOUD_LLM_CHUNK_TOKENS` | 3200 | 雲端萃取分塊大小（v4.3.3） | `config.py:82-85` |
| `CLOUD_LLM_MAX_CONCURRENT_REQUESTS` | 3 | 雲端萃取併發上限 | `config.py:86-89` |
| `LOCAL_LLM_EFFECTIVE_CONTEXT_TOKENS` | 8192 | 本地有效上下文預算（GPU compose 覆蓋為 16384） | `config.py:91-94` |
| `LOCAL_LLM_RESERVED_OUTPUT_TOKENS` | 3072 | 保留輸出預算（GPU compose 覆蓋為 2048） | `config.py:95-98` |
| `LOCAL_LLM_CHUNK_OVERLAP_LINES` | 4 | 分塊重疊行數 | `config.py:99-102` |
| `LOCAL_LLM_MAX_REFINEMENT_ROUNDS` | 2 | 補強輪上限（本地與雲端共用） | `config.py:103-106` |
| `LOCAL_LLM_KEEP_ALIVE` | `30m` | Ollama keep_alive（P0-7；v4.7.0 由 `10m` 調高） | `config.py:107-110` |
| `LOCAL_LLM_MAX_MERGE_ROUNDS` | 3 | 筆記整併輪上限 | `config.py:111-114` |
| `LOCAL_LLM_DISABLE_THINKING` | True | 關閉思考型模型 thinking（gemma4 正文為空的根因修復） | `config.py:115-118` |
| `LOCAL_LLM_REQUEST_TIMEOUT`（v4.6.2 新增） | 1800.0 | 每次 Ollama `/api/chat` 總時長上限（秒）；v4.7.0 改串流後為「寧可等待不截斷」的總上限 | `config.py:125-128` |
| `LOCAL_LLM_STREAM_IDLE_TIMEOUT`（v4.7.0 新增） | 120.0 | 串流回應 chunk 間最大閒置秒數，超過視為連線卡死（可重試） | `config.py:129-132` |
| `LOCAL_LLM_MIN_TOKENS_PER_SECOND`（v4.7.0 新增） | 5.0 | 生成吞吐低於此值即警告疑似 CPU offload | `config.py:133-136` |
| `LOCAL_LLM_DEGRADED_CONTEXT_TOKENS`（v4.7.0 新增） | 8192 | warmup 自我修復失敗時的任務級降級 `num_ctx` | `config.py:137-140` |
| `LOCAL_LLM_WARMUP_TIMEOUT`（v4.7.0 新增） | 600.0 | ASR 後 load-only 預熱請求專用逾時（秒） | `config.py:141-144` |
| `LOCAL_LLM_TRANSIENT_RETRIES`（v4.6.2 新增） | 2 | 每次本地 LLM 呼叫對瞬時錯誤（timeout/連線中斷）的額外重試次數 | `config.py:145-148` |
| `LOCAL_LLM_RETRY_BACKOFF_SECONDS`（v4.6.2 新增） | 5.0 | 瞬時錯誤重試的線性退避基數 | `config.py:149-152` |
| `CLOUD_LLM_REQUEST_TIMEOUT`（v4.6.2 新增） | 600.0 | Gemini（OpenAI 相容）請求逾時（秒） | `config.py:153-156` |
| `CLOUD_LLM_MAX_RETRIES`（v4.6.2 新增） | 2 | 雲端呼叫重試次數 | `config.py:157-160` |
| `ENABLE_TRANSCRIPT_CORRECTION` | True | 語意校正開關 | `config.py:165-168` |
| `CORRECTION_SCOPE` | `auto` | auto／all／off | `config.py:169-172` |
| `CORRECTION_MAX_SEGMENT_CHARS` | 400 | 校正分段長度 | `config.py:173-176` |
| `CORRECTION_CONTEXT_CHARS` | 60 | 校正附帶前文長度 | `config.py:177-180` |
| `CORRECTION_MAX_CHANGE_RATIO` | 0.10 | 單段改動上限，超過整段放棄 | `config.py:181-184` |
| `GLOSSARY_DIR` | 空（＝`data/glossary`） | 詞彙表目錄 | `config.py:185-188`、`glossary.py:23-29` |
| `ASR_BACKEND` | `auto` | auto／transformers／faster_whisper | `config.py:193-196` |
| `WHISPER_MODEL` | `MediaTek-Research/Breeze-ASR-26` | ASR 模型 | `config.py:197-200` |
| `WHISPER_MODEL_REVISION` | None（自動鎖 `949c87b…`） | revision 鎖定 | `config.py:201-204`、`asr_model_resolver.py:83-88` |
| `ASR_VAD_MIN_SILENCE_MS` | 500 | VAD 靜音分割門檻（P1-6 由 2000 調降） | `config.py:230-233` |
| `ASR_CONDITION_ON_PREVIOUS_TEXT` | False | 長檔防重複迴圈第一要務 | `config.py:243-246` |
| `ASR_ENABLE_HOTWORDS` | True | 詞彙表注入 ASR | `config.py:263-266` |
| `ASR_CHUNK_LENGTH_SECONDS` / `ASR_CHUNK_STRIDE_SECONDS` | 30 / 5 | transformers 重疊分塊解碼（P0-2） | `config.py:275-278`、`267-270` |
| `ASR_TRANSFORMERS_MIN_VRAM_MB` | 6000 | transformers 路徑最低可用 VRAM | `config.py:291-294` |
| `ASR_INITIAL_PROMPT` | 「以下是台灣繁體中文的會議記錄。」 | 繁體/標點誘導 | `config.py:295-298` |
| `ASR_ISOLATION`（v4.7.0 新增） | `subprocess` | ASR 執行隔離模式：`subprocess`＝獨立子程序（VRAM 保證歸還）／`inprocess`＝舊行為（回退桿） | `config.py:301-304` |
| `ASR_WORKER_TIMEOUT_SECONDS`（v4.7.0 新增） | 7200.0 | ASR 子程序硬上限（秒）；逾時 kill 並判定任務失敗 | `config.py:305-308` |
| `ALLOWED_ORIGINS` | `*` | CORS 來源（`*` 時自動停用 credentials） | `config.py:315-318` |
| `DATA_DIR` | `/app/data` | 資料根目錄 | `config.py:314` |
...
1. **ASR 開跑前**主動呼叫 `release_local_model()` 請 Ollama 卸載常駐模型（`task_processor.py:111-113`）。
2. `release_local_model`（`summarization.py:2010-2037`）：對 `/api/generate` 送 `keep_alive: 0` 請求卸載；**v4.3.2 根因修復**——Ollama 卸載是非同步的，若不等真的卸完，緊接著的 ASR 裝置偵測會看到 VRAM 仍被佔用而誤降級 CPU。因此輪詢 `/api/ps` 直到已載入模型清空，最多等 15 秒（每秒一次，`summarization.py:2027-2034`）。
3. 摘要階段 Ollama 以 `keep_alive=30m` 常駐（三階段連續十數次呼叫不重載 20GB 模型，P0-7；v4.7.0 由 `10m` 調高，`config.py:107-110`），流程結束後自然逾時釋放（`summarization.py:1175-1177`）。
4. **v4.7.0 ASR 子程序隔離進一步強化 VRAM 保證歸還**：上述 `/api/ps` 輪詢只能確認 *Ollama 自己*已卸載，無法保證 ASR 執行期間長駐程序累積的 CUDA context／記憶體分配器殘留完全釋回（faster-whisper#992 實測長駐程序每次殘留 ~312MB，逐任務蠶食 Ollama 之後可用的 VRAM）。每次轉錄改在獨立子程序執行（`backend/services/asr_subprocess.py:78-146`、`backend/workers/asr_worker.py`），程序退出時作業系統保證回收其全部 CUDA 配置，比任何 in-process 輪詢更確定；`ASR_ISOLATION=inprocess` 保留為回退桿（子程序啟動失敗時本任務自動退回，`asr_subprocess.py:100-105`）。詳見第 11.4 節與 ADR-9。
...
**背景**：ASR 前已強制卸載 Ollama 模型（第 11.2 節），故 ASR 完成後第一個生成呼叫必為冷啟動；且 Ollama scheduler 只在載入當下依可用 VRAM 決定 layer 配置——offload 一旦發生就會持續到下次重載，不會在推理過程中自行恢復。`task_processor.py:197-199` 在語意校正前呼叫 `warmup_local_model()`（`summarization.py:1925-2008`），把冷載入成本從生成呼叫的逾時額度中拆出，並嘗試自我修復：

| 步驟 | 動作 | 位置 |
| :--- | :--- | :--- |
| ① load-only 載入 | `/api/generate` 不帶 prompt，帶與後續 `/api/chat` 相同的 `options.num_ctx`（ctx 不一致會觸發整顆模型重載，白做 warmup）；專屬逾時 `LOCAL_LLM_WARMUP_TIMEOUT=600s` | `_load_model_and_check_offload`：`summarization.py:1859-1888` |
| ② offload 偵測 | 查 `/api/ps`，`size_vram < size` 即判定部分卸載至 CPU；`size_vram==0`（完全 CPU）另印專屬警告，指向「驅動更新/睡眠喚醒後 Ollama 長駐程序失去 GPU」的已知案例並建議重啟腳本 | `summarization.py:1955-1968` |
| ③ 自我修復（一次） | `release_local_model()` 卸載＋等待 2 秒＋重載；仍 offload 則判定修復失敗 | `summarization.py:1969-2001` |
| ④ 任務級降級 | 自我修復後仍 offload → `self._active_context_tokens = LOCAL_LLM_DEGRADED_CONTEXT_TOKENS`（8192）；以降級 ctx 立即重載一次，把重載成本吸收在 warmup 內（否則第一個 `/api/chat` 才因 ctx 改變觸發整顆模型重載） | `summarization.py:1983-1999` |
| ⑤ 降級值生效範圍 | `_effective_context_tokens()` 讀取順序：顯式覆蓋 > warmup 降級值 > settings 預設；每個任務 warmup 開頭重設為 `None`，防跨任務殘留 | `summarization.py:1110-1116`、`1940` |
| ⑥ KV 量化生效 heuristic | `/api/tags` 磁碟大小 vs `/api/ps` 載入大小之差 ≈ KV cache＋compute overhead；依 2026-08-19 正式機實測校準門檻（q8 生效 @16K overhead≈2.7GB；門檻依 `num_ctx` 線性縮放，兩門檻之間不下判斷以避免誤報） | `_check_kv_quantization_heuristic`：`summarization.py:1903-1923` |
...
`tests/` 共 **425 個 test 函式**（`grep -rc "def test_" tests/*.py` 實測，2026-08-20；經 parametrize 展開後 `pytest --collect-only` 收集為 **444 個**）。無 CI（`.github/workflows/` 為空目錄），以本機 `pytest` 執行。

| 測試檔 | 函式數 | 覆蓋重點 |
| :--- | :--- | :--- |
| `test_templates.py` | 62 | 會議模板註冊表、`FormLayoutSpec` 表單渲染、附件抽取（v4.4.0–v4.6.0 累積） |
| `test_api_routes.py` | 52 | 12 個 REST 端點成功/錯誤路徑 |
...
| `test_task_processor.py` | 16 | 管線總控、摘要失敗不偽裝成功、`summary_failed` 欄位 |
| `test_websocket_error_handling.py` | 8 | 斷線/例外清理 |
| `test_asr_subprocess.py`（v4.7.0 新增） | 7 | ASR 子程序隔離：worker 寫結果/進度、失敗 exit code、參數校驗、父程序轉發進度、逾時/失敗/spawn 失敗回退、`inprocess` 模式 |
| `test_transcription_service.py` | 6 | 轉錄結果組裝 |
| `test_queue_fix.py` | 4 | 佇列回歸修復 |
| `test_errors.py`（v4.6.2 新增） | 3 | `describe_exception`：一般訊息保留、空訊息 httpx/內建逾時例外回退類別名稱 |
| 其他（`install_deps`／`download_models`／`verify_env`／`timeout_middleware`／`device_detector`／`queue_integration`／`frontend_upload_flow`／`whisper_fix`／`end_to_end` 等） | 16 | 環境驗證、逾時中間件、整合冒煙 |
...
- **背景**：v4.7.0 把 Ollama 串流結束時所有非 `stop` 的 `done_reason` 一律視為可重試（`OllamaStreamRetryable`），移植自參考專案 `D:\dev\local` 的量產經驗。但本專案的筆記整併階段（`_merge_notes_until_fit`）刻意以 `merge_predict_cap` 綁定輸出上限（`summarization.py:724-727`），讓輸出撞到 `num_predict` 上限（`done_reason=length`）是**設計內行為**，用來保證多輪整併必然收斂（ADR-7）。
- **決策**：v4.7.1 把 `done_reason=length` 從可重試例外中除名——`_stream_ollama_chat_once`（`summarization.py:964-1024`）直接回傳已完整累積的內容；`_post_ollama_chat`（`summarization.py:1078-1085`）記錄 WARNING（含 `eval_count`/`num_predict`）後接受並繼續，不重試。
...
- **決策**：v4.7.0 新增 `backend/workers/asr_worker.py`（獨立可執行模組，`python -m backend.workers.asr_worker`）與 `backend/services/asr_subprocess.py`（`transcribe_isolated()`，父程序以 `asyncio.create_subprocess_exec` 管理，進度經 stdout JSON lines 轉發），把每次轉錄放進獨立子程序執行；`ASR_ISOLATION` 設定鍵保留 `inprocess` 作為回退桿（子程序啟動失敗時本任務自動退回 in-process，`asr_subprocess.py:100-105`）。
...
- **代價**：每次轉錄多一次程序啟動開銷與 stdout/stderr 管線管理複雜度（需同時讀取避免 pipe buffer 死鎖，`asr_subprocess.py:109-117`）；換得的是 VRAM 交接的確定性，對單卡 24GB 上「ASR 用完立刻讓給 20GB LLM」的場景是必要代價。
...
| 12 REST 端點 | `backend/api/routes.py:33-512`（明細見第 9.1 節表格，2026-08-20 已逐一核對） |
| /config 回傳生效模型名 | `backend/api/routes.py:424-451` |
| 下載檔名依 `summary_failed` 動態切換（v4.6.2） | `backend/api/routes.py:290`（`record_label`）、`333`、`364` |
...
| 例外訊息保底 `describe_exception`（v4.6.2 新增） | `backend/core/errors.py:10-13` |
| 管線總控 `_process_task` | `backend/services/task_processor.py:169-272` |
| 摘要失敗警告 banner | `backend/services/task_processor.py:32-35` |
| ASR 前釋放 Ollama VRAM | `backend/services/task_processor.py:111-113` |
| ASR 子程序呼叫入口（v4.7.0） | `backend/services/task_processor.py:117-123`（`transcribe_isolated`） |
| 本地 LLM 預熱呼叫點（v4.6.2） | `backend/services/task_processor.py:189-199`（`warmup_local_model`） |
| `_format_result`（驗證後不改本文） | `backend/services/task_processor.py:274-336` |
| 本地三階段管線 | `backend/services/summarization.py:845-932`（`_summarize_with_local_pipeline`） |
| 上下文預算規劃（P0-6） | `backend/services/summarization.py:278-326`（`_build_local_context_plan`） |
| 逐字稿分塊（重疊 4 行） | `backend/services/summarization.py:328-379`（`_split_oversized_line`／`_split_transcript_into_chunks`） |
| 本地萃取提示詞／雲端豐富度追加 | `backend/services/summarization.py:64-96`（本地）、`102-107`（雲端追加） |
| 筆記整併（三重收斂）＋硬截斷 | `backend/services/summarization.py:707-774`（`_merge_notes_until_fit`）、`776-794`（`_truncate_to_token_budget`） |
| 品質驗證（14 區塊＋召回） | `backend/services/summarization.py:419`（`_REQUIRED_SECTION_PATTERNS`）、`386-419`（待辦召回 key 抽取）、`421-476`（`_validate_summary_quality`） |
| 洩漏偵測（標籤/英文/評分準則） | `backend/services/summarization.py:478-540` |
| **地端管線韌性機制（v4.6.2–v4.7.1，詳見第 6.4／11.4 節）** | |
| 串流可重試例外類別 `OllamaStreamRetryable` | `backend/services/summarization.py:32-43` |
| 單次串流 `/api/chat`（閒置/總時長逾時、`done_reason=length` 判斷） | `backend/services/summarization.py:964-1024`（`_stream_ollama_chat_once`） |
| 生成指標觀測（load_duration/tokens-per-s/prompt_eval_count） | `backend/services/summarization.py:1027-1048`（`_log_generation_metrics`） |
| 瞬時錯誤重試＋think 相容降級＋`done_reason=length` 接受 | `backend/services/summarization.py:1050-1108`（`_post_ollama_chat`） |
| 任務級 num_ctx 生效順序 | `backend/services/summarization.py:1110-1116`（`_effective_context_tokens`） |
| Ollama 呼叫主流程（think:false、keep_alive、num_predict 自動擴大、重試） | `backend/services/summarization.py:1118-1246`（`_summarize_with_ollama`） |
| 模型名智能解析（4 策略） | `backend/services/summarization.py:1680-1742`（`_resolve_compatible_model`） |
| load-only 載入＋offload 偵測 | `backend/services/summarization.py:1859-1888`（`_load_model_and_check_offload`） |
| KV 量化生效 heuristic | `backend/services/summarization.py:1903-1923`（`_check_kv_quantization_heuristic`） |
| warmup 自我修復＋任務級降級 | `backend/services/summarization.py:1925-2008`（`warmup_local_model`） |
| VRAM 釋放＋/api/ps 輪詢 15s | `backend/services/summarization.py:2010-2037`（`release_local_model`） |
| ASR 子程序隔離（父程序管理／worker） | `backend/services/asr_subprocess.py:78-146`（`transcribe_isolated`）、`backend/workers/asr_worker.py:23-76`（`main`） |
| 雲端動態長度下限 | `backend/services/summarization.py:1354-1363`（`_estimate_cloud_min_summary_chars`） |
| 雲端分段併發萃取 | `backend/services/summarization.py:1365-1423`（`_extract_notes_with_gemini`） |
| 雲端兩段式主流程 | `backend/services/summarization.py:1425-1489`（`_summarize_with_gemini`） |
| 雲端雙輸入生成/補強訊息 | `backend/services/summarization.py:640-665`（`_build_cloud_summary_message`）、`667-684`（`_build_cloud_refinement_message`） |
| Gemini 串流（進度 86–94%） | `backend/services/summarization.py:1491-1528`（`_gemini_chat`）、`1530-1576`（`_gemini_chat_once`） |
| Gemini 健康檢查 24h 快取 | `backend/services/summarization.py:127-131`（快取初始化）、`1795-1857`（`check_gemini_health`／`reset_gemini_health_cache`） |
...
| 設定總表 | `backend/core/config.py:18-366`（明細見第 10.2 節表格） |
...
| 已知版本不一致 | `pyproject.toml:15`（4.2.0，未同步；以 `VERSION`＝4.7.1 為準） |
