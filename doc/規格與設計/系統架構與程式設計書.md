...
| 文件版本 | v4.9.0 |
| 文件日期 | 2026-09-22 |
| 系統版本 | 4.9.0（唯一來源：根目錄 `VERSION` 檔，經 `backend/core/version.py:15-26` 讀取） |
| 適用讀者 | 開發者、維護工程師 |
| 專案根目錄 | `D:\dev\convert` |
...
> v4.8.0 地端品質對齊（2026-09-22）——移除與 context 脫鉤的固定上限（分塊 3200／輸出 3072）、最終生成與補強改「筆記＋逐字稿」雙輸入、地端一體適用雲端紀錄契約（發言來源／年份依據／動態長度閘門／佔位符修復）、LM Studio 官方取樣參數——見第 6.4 節。
> v4.8.0 同步範圍：第 6.1／6.3／6.4、10.2、14、16 節的**新增機制**行號已依現行碼重新核對；`summarization.py` 於本版由 3,238 行增至 3,878 行，其餘既有行號沿用 v4.7.4 快照，未在本版重新保證新鮮度。
> v4.9.0 地端紀錄可查核性——出處標註時間戳確定性吸附（`snap_source_tags_to_transcript`，掛在同一條
> 地端後處理鏈 `_finalize_record_text(mode="local")`，`ollama`／`lmstudio` 共用、模型無關；fail-soft、
> 位移上限 120 s）與量測儀器強化（`measure_tag_traceability` 新增 `zero_time_tag_*`／`distinct_tag_time_*`／
> `on_start_tag_ratio_excluding_zero`）——見 CHANGELOG 與 `doc/規格與設計/地端會議紀錄品質對齊雲端-研究與優化規劃.md` §10。
> **本文行號仍為 v4.8.0 快照，v4.8.1／v4.9.0 未逐行重核**。
> v4.8.1 地端紀錄契約收斂（2026-09-22）——`_finalize_record_text` 分流 `mode="cloud"|"local"`（地端鏈：`finalize_record` → 術語修正 → 彙整表標註清除 → 佔位符修復 → general 跨節去重）、新增模板欄位 `MeetingTemplate.record_term_fixes`、驗收器模板感知與 `scripts/e2e/measure_record_quality.py`——見第 6.3 節末與 CHANGELOG。**本文行號為 v4.8.0 快照，v4.8.1 未逐行重核**（新增碼集中在`text_postprocess.py` 新函式與 `summarization.py` 的 `_finalize_record_text`，既有行號位移約 +50～+90）。
...
`backend/services/summarization.py`（3,878 行，v4.8.0 現況）是系統核心。設計哲學：**先萃取事實、再組合成公文**，而非把整份逐字稿直接生成公文——地端因上下文限制採分塊萃取（ADR-1）；雲端 v4.7.3 起預設單次呼叫整份逐字稿萃取、分段為可選回退（ADR-2）；v4.8.0 起地端與雲端共用同一套紀錄契約與記錄級後處理（第 6.3／6.4 節）。

### 6.1 本地路徑：`_summarize_with_local_pipeline`（`summarization.py:1974-2154`）

```mermaid
flowchart TD
    A["選擇引擎 _select_local_engine<br/>Ollama 優先，退 LM Studio<br/>(1892)"] --> B["上下文規劃 _build_local_context_plan<br/>(475)"]
    B --> C{"逐字稿 tokens ><br/>chunk 預算？"}
    C -->|"是"| D["分塊 _split_transcript_into_chunks<br/>依 speaker line＋自然斷點<br/>重疊 4 行／220-token 重疊預算 (864／242)"]
    C -->|"否"| E["整份視為單塊"]
    D --> F["階段① 逐塊萃取<br/>LOCAL_EXTRACTION_PROMPT<br/>輸出上限由 context 推導（v4.8.0）<br/>temperature 0.6"]
    E --> F
    F --> G["階段② 筆記收斂 _consolidate_notes (1546-1604)<br/>筆記放得下→零損串接（不呼叫 LLM，v4.8.0）<br/>放不下→_merge_notes_until_fit (1408) 有損整併<br/>三重收斂：輪上限 3／縮減停滯即停／<br/>單一 note 進入下游可承接預算即收斂（v4.7.4）<br/>永不硬截斷（超出下游預算＝fail loudly）"]
    G --> H["階段③ 最終生成<br/>筆記＋原始逐字稿雙輸入（v4.8.0；<br/>餘裕不足自動退回只餵筆記）＋公文 System Prompt<br/>輸出上限由 context 推導、temperature 0.7"]
    H --> I["記錄級後處理 _finalize_record_text<br/>(2157；含範本骨架佔位符修復，v4.8.0 起地端一體適用)"]
    I --> J["驗證 _validate_summary_quality<br/>(952；含動態長度閘門、發言來源、年份依據)"]
    J --> K{"有問題？"}
    K -->|"是，≤2 輪"| L["補強重寫 _resolve_final_refinement_message<br/>(1649；筆記＋逐字稿雙輸入，v4.8.0)"]
    L --> I
    K -->|"否／輪數用盡"| M["輸出（輪數用盡僅記 log 警告, 2136）"]
```

**上下文預算規劃**（`summarization.py:475-556`，`_build_local_context_plan`）：

| 預算 | 公式 | 位置 |
| :--- | :--- | :--- |
| `chunk_input_budget` | `min(max(1200, ctx − output − extraction_overhead), ceiling)`；ctx＝`LOCAL_LLM_EFFECTIVE_CONTEXT_TOKENS`（預設 8192），output＝`LOCAL_LLM_RESERVED_OUTPUT_TOKENS`（預設 3072）；**v4.8.0**：`ceiling` 改為 `LOCAL_LLM_CHUNK_INPUT_TOKENS_CEILING`（預設 0＝依 context 推導），移除與 context 脫鉤的固定 3200（小 context 的推導值與舊值幾乎相同） | `summarization.py:497-505` |
| 單次生成輸出上限（v4.8.0） | `_resolve_local_output_tokens`：`max(下限 3072, ctx − prompt − 安全邊界 256)`，再以 `LOCAL_LLM_OUTPUT_TOKENS_CEILING`（8192）封頂；萃取／最終生成／補強三個階段皆適用（舊行為三階段共用固定 3072，128K instance 只用掉 2.6%） | `summarization.py:557-588`；呼叫點 `2040`／`2076`／`2112` |
| 筆記收斂策略（v4.8.0） | `_consolidate_notes`：筆記總量 ≤ `merge_feasible_input` 時**零損串接**（`\n\n---\n\n`，完全不呼叫 LLM）；真的超出下游預算才走 `_merge_notes_until_fit` 有損整併。開關 `LOCAL_LLM_ZERO_LOSS_NOTES_PASSTHROUGH` | `summarization.py:1546-1604`、`1408` |
| 最終生成／補強輸入（v4.8.0） | `_resolve_final_generation_message`／`_resolve_final_refinement_message`：餵「筆記＋原始逐字稿」雙輸入（比照雲端）；`_final_message_fits` 以 context 餘裕判斷，放不下即自動退回只餵筆記。開關 `LOCAL_LLM_TRANSCRIPT_IN_FINAL_GENERATION` | `_final_message_fits`：`summarization.py:1606`；`_resolve_final_generation_message`：`1626`；`_resolve_final_refinement_message`：`1649` |
...
| :--- | :--- | :--- |
| 入口 | `_summarize_with_local_pipeline`（`summarization.py:1974-2154`） | `_summarize_with_gemini`（`summarization.py:2681-2755`，v4.7.3 起預設單次萃取） |
| 分塊依據 | 動態預算 `chunk_input_budget`（下限 1200；上限 v4.8.0 起由 context 推導，`summarization.py:497-505`） | 預設**不分塊**（單次整份萃取）；分段回退時固定 `CLOUD_LLM_CHUNK_TOKENS=3200`（`summarization.py:2453-2458`） |
...
| 筆記整併 | **零損串接優先**（v4.8.0 `_consolidate_notes`，`summarization.py:1546-1604`）——放得下就不整併；放不下才走 **有損整併** `_merge_notes_until_fit`（輪上限 3、停滯偵測、永不硬截斷，`summarization.py:1408`） | **零損串接** join（分段回退時；`summarization.py:2501`） |
| 最終生成輸入 | 筆記＋**原始逐字稿**雙輸入（v4.8.0；餘裕不足自動退回只餵筆記，`summarization.py:1626`） | 筆記＋**原始逐字稿**雙輸入（`summarization.py:2513-2521`） |
| 長度下限 | 動態 `max(250, min(2000, tokens//15))`（v4.8.0 起與雲端共用 `_estimate_cloud_min_summary_chars`，`summarization.py:3062`；舊行為固定 250 字） | 動態 `max(250, min(2000, tokens//15))`（`summarization.py:2419-2428`） |
| 補強輪 | ≤ `LOCAL_LLM_MAX_REFINEMENT_ROUNDS`（2），輸入＝筆記＋逐字稿（v4.8.0，`summarization.py:1649`） | 同上限 2 輪，輸入＝筆記＋逐字稿（`summarization.py:2530-2545`） |
...
共通不變式：**後處理（`_finalize_record_text`）永遠在驗證之前執行；驗證通過後任何流程不得再改寫本文**（P1-9，`summarization.py:2157`、`task_processor.py:302-310`）。v4.8.0 起地端與雲端共用同一套記錄級後處理（含範本骨架佔位符修復）與同一套紀錄契約絆索（動態長度閘門、發言來源標註、年份依據）。

> **v4.9.0 追加（2026-09-22）**：`mode="local"` 鏈在 `finalize_record` 之後加入 **出處標註吸附**
> （`snap_source_tags_to_transcript`：以逐字稿段落時間表把時間戳吸附到真實段落起點；規則 1 同發言者段落內、
> 規則 2 同發言者最近段落 ≤180 s、規則 3 落在任何真實段落內時只改時間不動歸屬、其餘原樣保留；
> `TAG_SNAP_MAX_SHIFT_SECONDS=120` 防「硬拉成 00:00:00」）。此步只改時間戳、不改發言者歸屬、不刪標註，
> 且**不含任何模型／引擎分支**，因此 LM Studio 與遠端 Ollama（Windows ＋ RTX 4090）一體適用。
> 實測（E2E attempt-B2-27b-fix、dense 27B）：`on_start_tag_ratio` 0.951 → **1.000**、
> `traceable_tag_ratio` 1.000、表格標註 0、verdict PASS（1,710 s，含 2 輪補強）。

> **v4.8.1 地端紀錄級後處理鏈（2026-09-22）**：`_finalize_record_text(summary, template=None, *, mode="cloud")` —
> `mode="cloud"`（預設）與 v4.8.0 完全相同（byte 級不變；實測 5 紀錄 × 4 模板 ＋ 72 提示詞案例與舊版相同）；
> `mode="local"` 依序為 `finalize_record` → `apply_record_term_fixes`（模板宣告的 `record_term_fixes`，目前僅
> `section_meeting` 4 條）→ `strip_source_tags_from_table_rows`（僅當 `forbidden_patterns` 標籤含「發言來源標註」
> 時啟用，只動 `^\s*\|` 表格列）→ `normalize_unfilled_placeholders` → `dedupe_cross_section_items`
> （僅 `general`，**嚴格最後一步**）。地端最終生成與**每一輪補強寫回**都走同一條（`summarization.py` 地端兩個呼叫點），
> 避免補強輪把已修掉的錯字／標註再寫回來。驗收儀器 `scripts/e2e/measure_record_quality.py`；runner 的 DOCX
> 正式性檢查改為模板感知（`TEMPLATE_REQUIRED_SECTIONS`，僅 `section_meeting`，其餘模板行為不變）。
> 實測（E2E attempt-04）：彙整表標註 13 → 0、正文標註 33 → 27、總耗時 439.5 s、verdict PASS。

### 6.4 地端管線韌性機制（v4.6.2–v4.8.0）
...
**v4.8.0（T20260922-1930-01）地端品質對齊雲端對策鏈**：使用者判定地端深度會議紀錄品質
「與雲端 Gemini 差距很大」。以同一支 `0903-科務會議.m4a`、同一份 ASR 逐字稿、同一
`section_meeting` 模板比對雲端 Gemini、地端 dense 27B（`qwen3.8-27b-splash`）與地端 MoE
35B-A3B（`qwen3.6-35b-a3b-splash`）三份輸出後確認：**差距的主因是管線，不是模型**。

| 根因 | 觀察事實 | 對策 | 位置 |
| :--- | :--- | :--- | :--- |
| RC-1 最終生成輸入不對稱 | 雲端餵「筆記＋完整逐字稿」，地端只餵整併後筆記；補強輪同理 | 地端最終生成與補強改雙輸入（`_resolve_final_generation_message`／`_resolve_final_refinement_message`），以 `_final_message_fits` 判斷餘裕，不足時自動退回只餵筆記；開關 `LOCAL_LLM_TRANSCRIPT_IN_FINAL_GENERATION` | `summarization.py:1606`／`1626`／`1649` |
| RC-2 與 context 脫鉤的固定上限 | `chunk_input_budget` 夾 3200、三階段輸出共用固定 3072、整併可見目標夾 4096；128K instance 只用約 2.6%，11,712 est 逐字稿被切 4 塊 | 分塊上限改 `LOCAL_LLM_CHUNK_INPUT_TOKENS_CEILING=0`（依 context 推導）；三階段輸出改 `_resolve_local_output_tokens`（下限 3072／上限 `LOCAL_LLM_OUTPUT_TOKENS_CEILING=8192`／安全邊界 256） | `summarization.py:497-505`、`557-588`；呼叫點 `2040`／`2076`／`2112` |
| RC-3 有損整併且損失被靜默接受 | 4 份筆記合計 8,176 tokens 送進整併，輸出上限 3,072 且 `finish_reason=length`（最壞損失 62.4%），流程視為「已收斂」，最終紀錄只承載逐字稿 15.7% | 新增 `_consolidate_notes`：筆記放得下就走**零損串接**（不呼叫 LLM），真的超出下游預算才整併；開關 `LOCAL_LLM_ZERO_LOSS_NOTES_PASSTHROUGH` | `summarization.py:1546-1604` |
| RC-4 雲端專屬機制地端缺席 | 發言來源標註（雲端 25 處／地端 0 處）、年份依據、動態長度閘門（雲端 `_estimate_cloud_min_summary_chars` vs 地端固定 250 字）、佔位符修復（地端未填佔位符 27 vs 雲端 14） | 地端驗證改套用同一組紀錄契約絆索（動態長度閘門＋發言來源＋年份依據），`_finalize_record_text` 納入範本骨架佔位符修復；遺漏待辦問題字串改列出具體項目（`ACTION_ISSUE_PREVIEW_LIMIT=12`） | `summarization.py:127`、`153-154`、`952`、`2157` |
| RC-5 取樣參數不完整 | LM Studio 路徑只送 `temperature`＋`max_tokens`（無 `top_p`／`top_k`），校正階段 `temperature=0.0` 等同 greedy | 送官方 Qwen3.6／3.8 非思考建議 `top_p=0.8`／`top_k=20` 與階段溫度（萃取 0.6／生成 0.7／補強 0.7／整併 0.6／校正 0.3）；Splash 引擎對 `min_p`／`presence_penalty` 非 0 值回 HTTP 400，故不送，端點拒絕取樣參數時逐級降級重送 | `_lmstudio_extra_body`／`_downgrade_extra_body`：`summarization.py:2876`／`2865`；`_lmstudio_chat_request`：`2898` |

**修正前診斷基準**（獨立 90 事實比對，同一逐字稿）：雲端 Gemini 覆蓋 61.7%／出處標註 25 處／
同音錯字修正 84.6%；地端 dense 27B 覆蓋 53.9%／0 處／53.8%／1,029.9 秒；地端 MoE 35B-A3B
覆蓋 75.6%（高於雲端）／0 處／15.4%／303.9 秒，另有 1 條捏造與 1 條自相矛盾。結論：覆蓋率
不是地端最弱的一環，**出處標註、同音校正、捏造抑制**才是——前者已由本版修正，後兩者列入
下一步建議。
...
> v4.8.0 註：`config.py:205-268` 新增 11 個 `LOCAL_LLM_*` 設定鍵（下表已標記 v4.8.0 者之新增列行號為現行值），使 `config.py` 205 行之後的既有行號整體位移；該段落其餘列沿用 2026-09-13 快照，未在本版重新核對。
...
| `LOCAL_LLM_CHUNK_INPUT_TOKENS_CEILING`（v4.8.0 新增） | 0 | 地端萃取分塊的每塊輸入上限；0＝依 context 推導（移除固定 3200） | `config.py:210-213` |
| `LOCAL_LLM_OUTPUT_TOKENS_CEILING`（v4.8.0 新增） | 8192 | 地端單次生成輸出天花板；實際值仍由 context 餘裕推導，且不低於 `LOCAL_LLM_RESERVED_OUTPUT_TOKENS` | `config.py:215-219` |
| `LOCAL_LLM_CONTEXT_SAFETY_MARGIN_TOKENS`（v4.8.0 新增） | 256 | 依 context 推導輸出預算時保留的安全邊界 | `config.py:220-223` |
| `LOCAL_LLM_TRANSCRIPT_IN_FINAL_GENERATION`（v4.8.0 新增） | True | 地端最終生成與補強是否同時餵入原始逐字稿（雙輸入，比照雲端）；餘裕不足自動退回筆記 | `config.py:224-228` |
| `LOCAL_LLM_ZERO_LOSS_NOTES_PASSTHROUGH`（v4.8.0 新增） | True | 筆記總量已被下游承接時改零損串接、略過有損 LLM 整併 | `config.py:229-231` |
| `LOCAL_LLM_SAMPLING_TOP_P` / `LOCAL_LLM_SAMPLING_TOP_K`（v4.8.0 新增） | 0.8 / 20 | LM Studio 取樣參數（Qwen3.6／3.8 官方非思考建議）；Splash 引擎不支援 `min_p`／`presence_penalty` 非 0 值，故不送 | `config.py:241-248` |
| `LOCAL_LLM_EXTRACTION_TEMPERATURE`／`_GENERATION_`／`_REFINEMENT_`／`_MERGE_`／`_CORRECTION_TEMPERATURE`（v4.8.0 新增） | 0.6／0.7／0.7／0.6／0.3 | 地端五個階段 temperature；校正由硬寫 0.0 改為設定（避免 Splash 引擎走官方警告的 greedy） | `config.py:249-268`、`task_processor.py:268` |
...
`tests/` 共 **732 個 test 函式**（`grep -rc "def test_" tests/*.py` 實測，2026-09-22；經 parametrize 展開後 `pytest --collect-only` 收集為 **821 個**）。無 CI（`.github/workflows/` 不存在），以本機 `uv run pytest tests/ -q` 執行；快速回歸排除真音檔 E2E（`tests/test_end_to_end.py`，1 項），v4.8.0 基準為 **818 passed／2 skipped／0 failed**（本次文件同步實測；2026-09-13 舊基準：730 passed／2 skipped）。
...
| `test_t20260922_regression.py`（v4.7.4 新增；v4.8.0 擴充為 9） | 9 | T20260922 回歸守衛：merge 可見目標由 context 推導（8192→900 下限、大 ctx→4096 上限）、單一 note 落在下游可承接預算內不得 veto、超出時仍 fail loudly、LM Studio `reasoning_effort=none` 與 HTTP 400 相容降級、規劃視窗以 instance `context_length` 為權威；v4.8.0 追加 LM Studio 送官方取樣參數（`top_p`／`top_k`）與端點全面拒絕時的逐級降級 |
| `test_t20260922_record_quality.py`（v4.8.0 新增） | 12 | 地端品質對齊契約（fail-first）：輸出上限由 context 推導、零損筆記串接與超出時才整併、最終生成／補強雙輸入與 context 不足自動退回、動態長度閘門＋發言來源＋年份依據、遺漏待辦問題列出具體項目 |
...
- **E2E 實測另行**：真實音檔端對端驗證以 `scripts/test_full_pipeline.py`、`tests/test_end_to_end.py` 與人工 E2E 驗收補足，不納入日常快速回歸；摘要修正可只重跑地端摘要階段（`scripts/e2e/rerun_local_summarize.py`，跳過 ASR／語意校正），並以 `scripts/e2e/check_record_output.py` 對輸出 MD／DOCX 做確定性驗收（非 fallback、正式標題、模板必備樣式、決議與待辦、長度、佔位符、DOCX 結構、中文占比；v4.7.4 新增）。Apple 路徑另有真機 E2E（44.9 分鐘 `tests/0903-科務會議.m4a` 與 27.7 分鐘 MP3，證據在 `.agent/tasks/T20260912-2242-01-apple-speech-analyzer-asr/e2e/`）。
...
| 本地三階段管線 | `backend/services/summarization.py:1974-2154`（`_summarize_with_local_pipeline`） |
| 上下文預算規劃（P0-6） | `backend/services/summarization.py:475-556`（`_build_local_context_plan`） |
| 地端單次生成輸出上限（v4.8.0） | `backend/services/summarization.py:557-588`（`_resolve_local_output_tokens`；呼叫點 `2040`／`2076`／`2112`） |
| 地端筆記零損串接／整併決策（v4.8.0） | `backend/services/summarization.py:1546-1604`（`_consolidate_notes`）、`1408`（`_merge_notes_until_fit`） |
| 地端最終生成／補強雙輸入（v4.8.0） | `backend/services/summarization.py:1606`（`_final_message_fits`）、`1626`、`1649` |
| 地端紀錄契約絆索（v4.8.0） | `backend/services/summarization.py:127`（`ACTION_ISSUE_PREVIEW_LIMIT`）、`153-154`（`RECORD_SPEAKER_TRACEABILITY_RULE`／`RECORD_DATE_GROUNDING_RULE`）、`952`（驗證）、`2157`（記錄級後處理） |
| LM Studio 取樣參數與降級（v4.8.0） | `backend/services/summarization.py:2865`（`_downgrade_extra_body`）、`2876`（`_lmstudio_extra_body`）、`2898`（`_lmstudio_chat_request`） |
...
| 已知版本不一致 | `pyproject.toml:15`（4.2.0，未同步；以 `VERSION`＝4.8.0 為準） |
