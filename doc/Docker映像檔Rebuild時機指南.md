...
本版本修改了以下檔案，**必須重建 Docker 映像檔**：

| 修改檔案 | 修改內容 | 影響 |
|----------|---------|------|
| `backend/services/summarization.py` | 移除自訂格式邏輯 | 簡化會議摘要生成 |
| `frontend/index.html` | 移除自訂格式 UI 區塊 | 簡化用戶界面 |
| `frontend/js/app.js` | 移除自訂格式上傳邏輯 | 簡化前端邏輯 |
